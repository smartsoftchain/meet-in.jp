...
class Adapter_AuthAdapter implements Zend_Auth_Adapter_Interface{
	...
	const ENC_SERVER_DOMAIN = "meet-in.jp";
	...
	public function authenticate(){
		...
		if(!empty($this->id) && !empty($this->password) || !empty($this->auto_login_key)){
			...
			// 特定企業の特定権限で使用できるメニューの制御フラグ
			$user["admin_header_enable"] = true;
			$user["room_in_menu_enable"] = true;
			// KCC株式会社(PFO)様(client_id=7063)の一般社員権限(staff_role=CE_2)は利用範囲を制限
			if($user["client_id"] == 7063 && $user["staff_role"] == "CE_2") {
				$user["admin_header_enable"] = false;
				$user["room_in_menu_enable"] = false;
			}
