...
error_log("★★★updateConnectionInfoPeerId user_peer_id_1(".$peerId.")\n", 3, "/var/tmp/negotiation.log");
	...
	public function updateRoomGuestLeaveRedirectUrl($id, $url) {
		$connectionInfoDao = Application_CommonUtil::getInstance("dao", "ConnectionInfoDao", $this->db);
		$connectionInfoDao->updateRedirectUrl($id, $url);
	}

	public function hasActiveUserOnRoom($id) {
		$connectionInfoDao = Application_CommonUtil::getInstance("dao", "ConnectionInfoDao", $this->db);
		return $connectionInfoDao->getOperatorCount($id) ? true : false;
	}

	public function getConnectionInfoById($id) {
 		$condition = "(id = '{$id}') ";
		$connectionInfoDao = Application_CommonUtil::getInstance("dao", "ConnectionInfoDao", $this->db);
		return $connectionInfoDao->getConnectionInfo($condition);
	}
...
//error_log("enterRoom connectionInfo=[".print_r($connectionInfo, true)."]\n", 3, "/var/tmp/negotiation.log");
	...
	private function pUpdateConnectionInfoPeerId($id, $userId, $peerId='X', $loginFlg=0, $roomModeFlg=0) {
