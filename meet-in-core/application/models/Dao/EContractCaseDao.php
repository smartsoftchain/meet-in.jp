...
class EContractCaseDao extends AbstractDao {
  ...
  */
  public function getPartnerCountApprovalStatusList($clientId) {
    ...
    $sql = "SELECT
      ...
      sum(CASE WHEN b.mail_sent_date IS NOT NULL THEN 1 ELSE 0 END) AS mail_sent_count
    FROM
  ...
  public function getEContractCaseList($clientId, $order, $ordertype, $page, $limit, $excludeCondition, $condition) {
    ...
    $sql = "SELECT
        ...
        FROM
          e_contract_case
        {$where_query} AND delete_date IS NULL AND update_date IS NOT NULL
        GROUP BY
          id
        ORDER BY
          {$order} {$ordertype}";
  ...
  */
  public function getConfirmingCountWithEmail($email, $excludeCondition, $condition) {
    $sql = "SELECT
       ...
       {$excludeCondition}
       {$condition} AND delete_date IS NULL AND update_date IS NOT NULL
       ;
     ";
    $rtn = $this->db->fetchRow($sql, array());
    return $rtn["count"];
  ...
  */
   ...
   * @return unknown
   */
  public function getEContractCaseCount($excludeCondition, $condition) {
