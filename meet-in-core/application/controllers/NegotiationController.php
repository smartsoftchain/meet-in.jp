...
class NegotiationController extends AbstractController
{
    ...
    public function negotiationVideoListAction()
    {
        ...
        $this->_loginCheck();
			...
			if (($this->user["client_id"] != "1" && (!in_array($staffLogin['staff_type'], [self::AUTH_AA, self::AUTH_CE]))) 
					|| $this->user["record_method_type"] != "0")
			{
				$this->_redirect('/index/menu');
			}
	 ...
	 * PDF出力する機能
	 */
	public function downloadShareMemoAction()
	{
		// POSTデータの読み込み
		$form = $this->_getAllParams();

		$this->_logger->debug("downloadShareMemoAction From=[".$form["shareMemo"]. "]");
		// 共有メモをテキスト形式でダウンロードする
		$fileName = "share_memo".date("Ymdhis").".txt";
...
//$this->_logger->info(json_encode($form));
	...
	public function roomAction()
	{
		...
		// 電子契約テンプレート取得
		if ($this->user['staff_role'] != "AA_3" && $this->user['staff_role'] != "CE_3" && $is_e_conteact_credential == 1 && $is_e_conteact_sign_image == 1 && $is_e_conteact_certificate == 1) {
		...
		if (isset($staff)) {
			...
			// ここで退出時のリダイレクトURLを登録する
            if (!$apiModel->hasActiveUserOnRoom($params['connection_info_id'])) {
				$masterClientDao = Application_CommonUtil::getInstance("dao", "MasterClientDao", $this->db);
				$condition = 'AND client_id = '. $staff['client_id'];
				$client = $masterClientDao->getClientList($condition, "client_id", "", 1, 1);
				if (count($client) && $client[0]['client_redirect_url'] != "") {
					$apiModel->updateRoomGuestLeaveRedirectUrl($params['connection_info_id'], $client[0]['client_redirect_url']);
				} else {
					$apiModel->updateRoomGuestLeaveRedirectUrl($params['connection_info_id'], '');
				}
            }
		...
		if (empty($params["user_id"])) {	// USER_ID なし
			...
			$result = $apiModel->enterRoom($connectionInfo, $collabo_site, $room_mode_flg, $locked_token);
	...
	public function viproomAction()
	{
		...
		// 電子契約テンプレート取得
		if ($this->user['staff_role'] != "AA_3" && $is_e_conteact_credential == 1 && $is_e_conteact_sign_image == 1 && $is_e_conteact_certificate == 1) {
			$eContractApiModel = Application_CommonUtil::getInstance("model", "EContractApiModel", $this->db);
