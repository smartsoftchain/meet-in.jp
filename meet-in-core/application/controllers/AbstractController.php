...
abstract class AbstractController extends Zend_Controller_Action
{
	...
	const STAGE_SERVER = "stage.meet-in.jp";
	const DEV_SERVER   = "dev3.meet-in.jp";
	const DEV_SENSE_SERVER = "163.43.31.77";
	const DEV_AIDMA_SERVER = "27.133.150.135";
	const AIDMA_DEV_SERVER = "192.168.33.12";
	...
	public function init(){
		...
		$this->view->osName = $this->getOSName();
		...
		if (is_null($alreadyReadNotificationsArray)) {
			$alreadyReadNotificationsArray = [];
		}
		if (is_null($unreadNotificationCount)) {
...
*/
		...
		} elseif(strpos($_SERVER["REQUEST_URI"], "get-audio/record-update")) {
		...
		if ($webinarModel->clientIdentification($this->view->user)) {
			// AA or ウェビナー可能時間が0以外ならはじく
			$this->view->webinar_available_time = $webinarModel->clientIdentification($this->view->user);
		}
...
// 			}
	...
	public function getOSName() {
		...
		// mac
		else if (preg_match('/OS X/', $userAgent)) {
			$osName = "Mac";
		}
		// Android
	...
	public function getBrowserType() {
		...
		// Firefox
		else if (preg_match('/Firefox/', $_SERVER['HTTP_USER_AGENT']) && !preg_match('/Seamonkey/', $_SERVER['HTTP_USER_AGENT'])) {
			$browserType =  "Firefox";
		}
		// Seamonkey
	...
	// ブラウザの種類を取得
	// getBrowserType()の判定が正しくないが、テンプレートで使用されていて動きが変わってしまうため新しいメソッドにする
	// 2020.05.23
	public function getBrowserTypeNew() {
		// 判定するのに小文字にする
		...
		elseif (strstr($browser, 'OPR') || strstr($browser, 'Opera')) {
			$browserType =  "Opera";
		}
