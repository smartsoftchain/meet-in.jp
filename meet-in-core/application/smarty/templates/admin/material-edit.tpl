...
$(function (){
	...
	function validFileInput(files) {
		...
		if ($("#materialname").val().length == 0 ){
				let fileName = getFileNamesRemoveExtensions(files[0].name);
			if (!(fileName.length <= 64) ){
				$('#document_drop_help').text('資料名は64文字以内で入力してください');
			} else {
	...
	function validMovInput(files) {
		...
		if ($("#materialname").val().length == 0 ){
				let fileName = getFileNamesRemoveExtensions(files[0].name);
			if (!(fileName.length <= 64) ){
				$('#mov_drop_help').text('資料名は64文字以内で入力してください');
			} else {
				$("#materialname").val(fileName);
			}
		}
				...
				<table class="mi_table_input_right mi_table_main">
					<tbody>
						...
						<tr>
							...
							<td class="mi_tabel_content">
								{if $material.material_id == null}
									...
									<label for="materialtype0" class="radio_character mgr_25">資料データ</label>
									...
									<label for="materialtype1" class="radio_character mgr_25">URL</label>
						...
						<tr id="material_type_file">
							...
							<td class="mi_tabel_content">
								<div id="document_drop_area" {if $browser == "Edge"}style="border: none;"{/if}>
									<p id="document_drop_label">{if $browser != "Edge"}資料データをここにドロップしてください{/if}</p>
									<p class="document_drop_message document_attention">※資料名が未入力の場合、アップロードしたファイル名がそのまま資料名に反映されます</p>
						...
						<tr id="material_type_mov">
							...
							<td class="mi_tabel_content">
								<div id="mov_drop_area" {if $browser == "Edge"}style="border: none;"{/if}>
									<p id="mov_drop_label">{if $browser != "Edge"}資料データをここにドロップしてください{/if}</p>
									<p class="document_drop_message document_attention">※資料名が未入力の場合、アップロードしたファイル名がそのまま資料名に反映されます</p>
						...
						<tr>
							...
							<td class="mi_tabel_content">
								<input type="radio" id="documentguardkeyflg0" name="document_guard_key_flg" value="0" class="radio_material_common" {if $material.document_guard_key_flg == 0}checked{/if} />
								<label for="documentguardkeyflg0" class="radio_character mgr_60">無</label>
								<input type="radio" id="documentguardkeyflg1" name="document_guard_key_flg" value="1" class="radio_material_common" {if $material.document_guard_key_flg == 1}checked{/if} />
								<label for="documentguardkeyflg1" class="radio_character">有</label>
								<div class="clear_both"></div>
							</td>
						</tr>
						<tr>
