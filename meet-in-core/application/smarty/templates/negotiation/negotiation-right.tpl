  ...
  <div class="record_modal_message_wrap">
    <div class="record_message">
      ...
      <div style="margin-top:10px">
        有効にする方法は<a href="javascript:void(0);" onclick="window.open('https://manual.meet-in.jp/?p=1297#129732', '_blank');" style="text-decoration:underline;">こちら</a>
      </div>
    </div>
  ...
  <ul id="negotiation_right_menu_button_list" style="display:none">
    ...
    {if $is_operator == 1}
      {if (!empty($user.client_id) && ($user.staff_role == "AA_1" || $user.staff_role == "CE_1" || $user.staff_role == "CE_2" ) && $user.plan_this_month > 1) || $user.trialUserFlg == 1}
        <!-- 特定企業の特定権限では非表示 -->
        <li class="show_material_modal {if $room_mode == 2 || !$user.room_in_menu_enable}view_only_control_footer{/if}" id="show_material_modal" >
          <span class="icon-memo"></span>
          <span>資料追加</span>
        </li>
      {/if}
      <!-- ECC様(client_id=1821)のみ、アルバイト権限で資料追加を利用可能にする start -->
      {if $user.client_id == 1821 && $user.staff_role == "CE_3"}
        <li class="show_material_modal {if $room_mode == 2}view_only_control_footer{/if}" id="show_material_modal" >
          <span class="icon-memo"></span>
          <span>資料追加</span>
        </li>
      {/if}
      <!-- ECC様(client_id=1821)のみ、アルバイト権限で資料追加を利用可能にする end -->
    ...
    {/if}
    <!-- 特定クライアントの特定権限では非表示 -->
    <li id="button_calendar" {if $room_mode == 2 || (isset($user.room_in_menu_enable) && !$user.room_in_menu_enable) }class="view_only_control_footer"{/if}>
    ...
    <li id="direct_sms_send_in_room" {if $room_mode == 2 || $is_operator != 1 }class="view_only_control_footer"{/if}>
      <span id="direct_sms_icon"><img src="/img/lp/ico-sms.svg"></span>
      <span>SMS送信</span>
    </li>
    {if $is_operator == 1}
