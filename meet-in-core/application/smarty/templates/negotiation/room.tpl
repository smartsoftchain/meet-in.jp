...
<html lang="ja">
	...
	{/if}
		...
		<div id="mi_wrap">
			...
			<input type="hidden" id="staff_role" name="staff_role" value="{$user.staff_role}"/>
			...
			{else}
			<!-- safariでは既存の背景合成を使用する可能性を考慮し、コメントアウト -->
			<!-- 併用して背景合成を行った場合、背景合成の映像が本来表示される映像の左上に小さく表示されてしまう為、netogiation_bodypix.jsに手を加える必要性有 -->
			...
			<script type="text/javascript" src="/js/negotiation/negotiation_recording.js?{$application_version}"></script>
