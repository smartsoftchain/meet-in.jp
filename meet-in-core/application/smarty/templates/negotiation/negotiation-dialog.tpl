...
<div class="modal-content" id="modal-content-requirements_browser">
	...
	<div class="mi_gest_close_mesg mi_modal_type_m" style="background-color: white;padding: 0 35px;">
	  <h2 style="text-align: center;  margin-bottom: 20px;">Internet Exploreは推奨ブラウザではございません。</h2>
	  <p>
	    Internet Exploreのサポート終了に伴い、<br>
		ご利用頂く事はできるのですが、セキュリティ維持のため<br>
	    当社の方でのサポートもできかねてしまう状況でございます。<br>
	    ※推奨ブラウザであるchromeでの再接続をお願いいたします。
	  </p>
...
<div id="loading-upload-modal-template" class="modal-content"  type="text/x-handlebars-template">
	...
	<div class="modal-body">
		...
		<div>
			<h2 class="title">現在録画ファイルをアップロード中です</h2>
			<div class="text-wrap">
				<p>
				...
				<div class="info_area_wrap">
					<div class="info_area">
				        ...
				        <table class="mi_table_input_right mi_table_main">
									<tbody>
										<tr class="download_url_wrapper">
											<td class="mi_tabel_title">
												●録画ダウンロードURL<br>
											</td>
											<td id="download_url" class="mi_tabel_content">
												<p><span class="alert_message">※画面右上の「閉じる」アイコンまたは画面下部の「登録する」ボタンをクリックしない場合は、ダウンロードできませんのでご注意ください。</span></p>
												<p class="reminder"><span class="alert_message">※録画データの保存期間は10日間です。</span></p>
												<p><a href="" id="download_url_body" target="_blank" rel="noopener noreferrer"></a></p>
										...
										<tr>
											...
											<td class="mi_tabel_content">
												<textarea name="chat_contents" rows="3" cols="35" class="mi_default_input" ></textarea>
											</td>
										</tr>
					...
					<div class="info_area">
				        ...
				        <table class="mi_table_input_right mi_table_main">
									<tbody>
										...
										<tr>
											<td class="mi_tabel_title">●シークレットメモ</td>
											<td class="mi_tabel_content">
		...
		<div class="modal_material_header">
			...
			<div style="display:flex;align-items: center;">
				...
				<div class="search_box">
						<input type="text" class="text_search" name="free_word" value="{{freeWord}}" placeholder="ファイル名を入力してください" class="mi_active">

						<button class="mi_default_button search_button">
			...
			<div class="remarks">※ルーム内にて資料を展開後、ダウンロードの可否を選択できます。</div>

		</div>
		<div class="modal_material_list_area">
			{{#each materialBasicList}}
			<div class="modal_material" id="{{material_id}}">
				<input type="checkbox" name="checked_materials[]" value="{{material_id}}" class="material_check"/>
					...
					<div class="modal_material_info">
						{{#if type_0}}
							資料：<span class="material_name">{{material_name}}</span><br>
						{{/if}}
						{{#if type_1}}
							URL：<span class="material_name">{{material_name}}</span><br>
						{{/if}}
						{{#if type_2}}
							画像：<span class="material_name">{{material_name}}</span><br>
						{{/if}}
						{{#if type_3}}
							画像：<span class="material_name">{{material_name}}</span><br>
						{{/if}}
...
{/literal}
{literal}
<!-- ====================================== 資料共有設定モーダルコンテンツ[start] ====================================== -->
<script id="material-share-setting-modal-template" type="text/x-handlebars-template">
	<div class="list-area hgt3 mi_modal_default material_modal_area" style="height: calc(100% - 20px);">
		<div class="modal_material_header">
			<span class="icon-document-01"></span>　資料情報設定
		</div>
		<form method="post" enctype="multipart/form-data">
		<div class="modal_material_list_area">
			{{#each materialSelectedListForSetting}}
			<div class="modal_material_setting" id="{{material_id}}">
				<div class="modal_material_info">
					<div class="material_type_url">
						<span>資料:</span> {{material_name}}
					</div>
					</br>
					<div class="material_type_file_dl">
						<span>ダウンロード表示 </span>
						<div class="download_select_area">
							<input type="radio" name="download_flg[{{material_id}}]" value="1" class="radio_material_{{material_id}} input_radio"/>
							<label for="downloadflg1_{{material_id}}" class="radio_character mgr_60 input_radio">あり</label>
						</div>
						<div class="download_select_area">
							<input type="radio" name="download_flg[{{material_id}}]" value="0" class="radio_material_{{material_id}} input_radio" />
							<label for="downloadflg0_{{material_id}}" class="radio_character input_radio">なし</label>
						</div>
							<input type="hidden" name="client_id[{{material_id}}]" value="{{client_id}}" />
							<input type="hidden" name="material_type[{{material_id}}]" value="{{material_type}}" />
						<div class="clear_both"></div>
					</div>
				</div>
			</div>
		...
		<div class="btn_area" align="center">
			<button type="button" name="material_cancel" class="mi_default_button mi_cancel_btn mi_btn_m hvr-fade">キャンセル</button>
			<button type="button" name="material_select_setting" class="mi_default_button mi_btn_m hvr-fade">共有</button>
		</div>
	</div>
</script>
<!-- ====================================== 資料共有設定モーダルコンテンツ[end] ======================================== -->
...
{literal}
<!-- ====================================== 資料ズームモーダルコンテンツ[start] ====================================== -->
<script id="material-zoom-modal-template" type="text/x-handlebars-template">
	<div class="list-area hgt3 mi_modal_default material_modal_area" style="width: 520px; height: 500px;">
		<div class="modal_material_header">
			<span class="icon-expansion"></span>　資料拡大
		</div>
		<div class="info_area_wrap">
			<div class="info_area" style="margin:45px 0px 40px 80px;">
				<div style="line-height: 30px;font-size:15px">倍率：</div>
					<ol style="line-height: 30px; list-style-type:none;">
					<li><input type="radio" id="zoom300" name="materia_zoom_type" value="300"><label for="zoom300">300%</label></li>
					<li><input type="radio" id="zoom200" name="materia_zoom_type" value="200"><label for="zoom200">200%</label></li>
					<li><input type="radio" id="zoom150" name="materia_zoom_type" value="150"><label for="zoom150">150%</label></li>
					<li><input type="radio" id="zoom125" name="materia_zoom_type" value="125"><label for="zoom125">125%</label></li>
					<li><input type="radio" id="zoom100" name="materia_zoom_type" value="100"><label for="zoom100">100%</label></li>
					<li><input type="radio" id="zoom999" name="materia_zoom_type" value="999"><label for="zoom999">カスタム</label> <input type="text" name="materia_zoom_value" size="4" value="{{zoomVal}}"> ％</li>
					</ol>
			</div>
		</div>
		<div class="btn_area" align="center">
			<button type="button" name="material_cancel" class="mi_default_button mi_cancel_btn mi_btn_m hvr-fade">キャンセル</button>
			<button type="button" name="material_zoom_select" class="mi_default_button mi_btn_m hvr-fade">決定</button>
		</div>
	</div>
</script>
<!-- ====================================== 資料ズームモーダルコンテンツ[end] ======================================== -->
