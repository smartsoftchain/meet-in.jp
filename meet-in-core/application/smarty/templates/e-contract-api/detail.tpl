          ...
          <div class="form_setting_document_img" id="0">
            <div class="form_setting_document_operation_area2">
              <div class="form_setting_document_operation_left">
                ...
                <div class="form_setting_document_operation_day">
                  {if $rejectedCount != 0}
                    {foreach from=$partnerList item=partner name=partnaerInfo}
                      {if $smarty.foreach.partnaerInfo.first}
                        <div>送信日：{$partner.mail_sent_date}</div>
                      {/if}
                      {if $partner.approval_status == 2}
                        <div>却下日：{$partner.update_date}</div>
                      {/if}
                    {/foreach}
                  {elseif $case.uid != null}
                    {foreach from=$partnerList item=partner name=partnaerInfo}
                      {if $smarty.foreach.partnaerInfo.first}
                        <div>送信日：{$partner.mail_sent_date}</div>
                      {/if}
                      {if $smarty.foreach.partnaerInfo.last && $partner.approval_status == 1}
                        <div>締結日：{$partner.update_date}</div>
                      {/if}
                    {/foreach}
                  {else}
                    {foreach from=$partnerList item=partner name=partnaerInfo}
          ...
          <div class="form_setting_partner_item">
            ...
            <div class="form_setting_partner_name">
              ...
              {if $partner.approval_status == 1}
                <div>承認日：{$partner.update_date}</div>
            ...
            {elseif $partner.approval_status == 2}
              <div class="form_setting_partner_resend">却下</div>
            {else}
              {if $resendFlg == 0}
