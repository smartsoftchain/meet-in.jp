 ...
 */
try{
	//debugMeg("ConvNumConsumerMaster_begin");
	...
	$consumerList = getConsumerMasterByRelationId($relationId);
	//error_log("[ConsumerMasterCount:".count($consumerList)."][date:".date("Y-m-d H:i:s")."]");
	$consumerRowCount = 0;
...
// 		error_log("=============================");
		...
		if(count($sqlParts) > 0){
			...
			$sql = "UPDATE consumer_master SET {$strSqlParts} WHERE id={$consumer["id"]};";
			fwrite($fp, $sql);
			// 更新対象のカウント
			$consumerRowCount++;
		}
	...
	fclose($fp);
	//error_log("[UpdateConsumerMasterCount:".$consumerRowCount."][date:".date("Y-m-d H:i:s")."]");
	//debugMeg("ConvNumConsumerMaster_end");
}
catch (Exception $err){
 ...
 * 全コンシューマーマスターを取得する
 */
function getConsumerMasterByRelationId($relationId) {
