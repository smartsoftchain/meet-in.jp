 ...
 */
try{
	debugMeg("UpdateAudioTextLimitTime begin");
	...
	try {
		...
		$sql = "update master_client_new set negotiation_audio_text_time_limit_second = negotiation_audio_text_remaining_hour * 3600;";
		$db->query($sql);
		$db->commit();
	}catch(Exception $e){
		$db->rollback();
	...
	debugMeg("UpdateAudioTextLimitTime end");
}
catch (Exception $err){
