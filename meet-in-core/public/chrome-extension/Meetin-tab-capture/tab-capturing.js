	...
	chrome.storage.sync.get(null, function(items) {
		...
		chrome.tabCapture.capture(
			...
			function(stream) {
				...
				startCaptureTabCaptureSub(stream, port);
...
function startCaptureTabCaptureSub(videoStream, port) {
	var audio = new Audio();
...
//		outputTracks = outputTracks.concat(audioStream.getAudioTracks());
		...
		mTabCaptureRecorder = new MediaRecorder(mTabCaptureStream, options);
...
function uploadChunkedMedia(chunk) {
	...
	try {
		...
		xhr.open('POST', `https://meet-in.jp/negotiation/upload-chunk-to-temporary-bucket`, false);
		xhr.onload = (e) => {
			console.log('upended');
			chunkEndedFlg = true;
		};
...
function onEndCompose(port) {
	...
	try {
		...
		while (!chunkEndedFlg) { // もしもchuckUploadが終わってない場合は、30秒待つ処理を続ける
			...
			// 拡張機能側はホストが別なのでハードコーディングしている。
			xhr.open('POST', `https://meet-in.jp/negotiation/compose`, true);
