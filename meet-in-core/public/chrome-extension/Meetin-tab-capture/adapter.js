...
AdapterJS.TEXT = {
  PLUGIN: {
    REQUIRE_INSTALLATION: 'meet inを動作させるのにTemasysが提供しているWebRTCのプラグインをインストールする必要があります。',
    NOT_SUPPORTED: 'お使いになるブラウザはWebRTCをサポートしていません。',
    BUTTON: 'インストール'
  },
  REFRESH: {
    REQUIRE_REFRESH: 'ページを更新',
    BUTTON: '更新'
  }
};
...
AdapterJS.renderNotificationBar = function (text, buttonText, buttonLink, openNewTab, displayRefreshBar) {
  ...
  if(buttonText && buttonLink) {
    c.document.write('<button id="okay">' + buttonText + '</button><button id="cancel">キャンセル</button>');
    c.document.close();
