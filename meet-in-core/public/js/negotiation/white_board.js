...
var whiteBoardHeight = 676;			// キャンバスのサイズ丈
...
$(function () {
	...
	// サイズ変更や移動に関するイベント
	// ===================================================================
	// ホワイトボードの領域を移動できるようにイベント処理を追加
	$("#white_board_area").draggable({ cancel: '.canvas_area', containment: "#mi_video_area", scroll: false});
	
	// ホワイトボードを移動した場合(オペレーターのみ操作可能)
	$("#white_board_area").on("drag", function() {
		var data = {
				command : "WHITE_BOARD", 
				type : "MOVE_WHITE_BOARD", 
				top : $(this).css("top"), 
				left : $(this).css("left") 
		};
	...
	// ホワイトボードのリサイズ処理
	$("div#white_board_area").resizable({
		minHeight: 370,
		minWidth: 280,
		maxHeight: 850,
		maxWidth: 1320,
	...
	// キャンバス押下時の処理
	$(document).on('mousedown', '#white_board', function(e){
		// マウス押下のフラグを立てる
	...
	$(document).on('mouseup', '#white_board', function(){
	...
	$(document).on('mouseout', '#white_board', function(){
		whiteBoardOnMouseFlg = false;
		onMouseX = 0;
		onMouseY = 0;
		saveWhiteBoardImage();
	});
	// タッチデバイス用のタッチ終了時の処理(オペレーターのみ操作可能)
	$('#white_board').bind('touchend', function() {
		whiteBoardOnMouseFlg = false;
		onMouseX = 0;
		onMouseY = 0;
		saveWhiteBoardImage();
	...
	// キャンバス押下後移動時の処理(オペレーターのみ操作可能)
	$(document).on('mousemove', '#white_board', function(e){
	...
	// タッチデバイス用のキャンバス押下後移動時の処理(オペレーターのみ操作可能)
	$(document).on('touchmove', '#white_board', function(e){
			// 描画可能箇所にタッチして、移動している場合（タッチ描画している時）のみ処理を行う
			if(whiteBoardOnMouseFlg && (selectLeftTool == TOOL_PEN_NORMAL || selectLeftTool == TOOL_PEN_HIGHLIGHT || selectLeftTool == TOOL_PEN_ERASER)){
				// 要素の位置を取得
				var element = document.getElementById("white_board");
				var rect = element.getBoundingClientRect();
	...
	// ===========================================================
	// 閉じるボタン押下時のイベント
	$(document).on('click', 'div.white_board_header_icon_area .btn_close', function(){
