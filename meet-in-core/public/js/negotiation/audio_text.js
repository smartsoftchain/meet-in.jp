...
$(function ()
{
	...
	$('#audio-text-start').on('click', function() {
		...
		if(!sentimentAnalysisFlg){
			// 音声分析が未実行の場合のみ、文字起こしが実行できる TODO 機能制限
			...
			// conversationIdが取得できてから文字起こし機能を開始する
			startAudioTextInfo();
		}else{
			// 音声分析停止のメッセージ表示
	...
	/**
	 * conversationIdが取得できてから文字起こし機能を開始する様にする関数
	 */
	function startAudioTextInfo(){
		setTimeout(function (){
			// conversationIdを判定し、nullだった場合は再帰する
			if(NEGOTIATION_RECORDER.getFileID() 
			// ホストのマイクが使用不可の場合でもゲストがいる場合は文字起こしを開始する
			|| (mNegotiationMain.getTotalTargetPeerData() > 0 && NEGOTIATION.isMyMicError == true)){
				audioTextInfo(function(data) {
					...
					recognizeStartTimeMS = onRecognizeStartFunc();
