...
//	console.log(JSON.stringify(staff_type) + ':' + JSON.stringify(staff_id) + ':' + JSON.stringify(client_id)  + ':' + JSON.stringify(user_id));
	if(staff_type != null && staff_id != null && client_id != null) {
		...
		let my_name = '';
		if (valueCheck($('#my_user_info').val())) {
			my_name = $('#my_user_info').val();
		} else if (valueCheck($('#user_name').val())) {
	...
	if($("#room_mode").val() == ROOM_MODE_1){
		...
		// null の場合は使用不可
		if (NEGOTIATION.isMyCameraError == true || NEGOTIATION.isMyMicError == true) {
			//$('.remind_camera_on_off_tooltip').css('visibility', 'hidden');
		}
	}else{
		// 明示的にマイクをOFFにする
		micOff();
