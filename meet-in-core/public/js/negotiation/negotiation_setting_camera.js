...
console.log("OLD_facing:[" + NEGOTIATION.use_camera_facingMode + "]");
	if( NEGOTIATION.use_camera_facingMode ) {
		...
		} else if(USER_PARAM_IS_IPAD_PC) {
			// 必ず切り替えるわけではないのでlabelを見て設定
			let use_camera_name = $('#setting-camera-dialog-camera-list option:selected').text();
			NEGOTIATION.use_camera_facingMode = use_camera_name === "背面カメラ" ? "environment" : "user";
			console.log("カメラIDが切り替:("+ NEGOTIATION.use_camera_facingMode +")");
		}
	}
	...
	var beautyMode = localStorage.getItem('beauty_mode');
...
function settingBeautyMode(targetUserId, beautyMode){
	...
	var hideBackground = '#myCanvasHideBackground';
	// ビューティーモード判定
	if(beautyMode == BEAUTY_MODE_ON){
		// ========================
		// ビューティーモードON
		...
		if(0 < $(hideBackground).length) {
			$(hideBackground).css('filter','brightness('+BEAUTY_MODE_BRIGHT_VALUE+'%)');
			$(hideBackground).css('object-fit','fill');
		}
	}else{
		// ========================
		// ビューティーモード OFF
		...
		$('#video_target_video_' + userId).css('filter','brightness('+brightVal+'%)');
		if(0 < $(hideBackground).length) {
			$(hideBackground).css('filter','brightness('+brightVal+'%)');
			$(hideBackground).css('object-fit','cover');
		}
...
function setBrightnessVal(brightness) {
	...
	if (brightness) {
		...
		// 背景隠し中の場合.
		if(0 < $(hideBackground).length) {
			$(hideBackground).css('filter','brightness('+val+'%)');
		}
	}
