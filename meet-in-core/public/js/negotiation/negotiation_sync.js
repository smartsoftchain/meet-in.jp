...
function receiveSyncJson(json){
	if(json.type == "REQUEST_SYNC"){
		...
		// チャットメモの位置を設定
		$("div#chat_board_area").css("top", json.top);
		$("div#chat_board_area").css("left", json.left);
		// チャットの大きさを設定
		...
		if(json.showFlg){
			...
			// チャットのスクロール位置を設定
			$("div#chat_board_area").scrollTop(json.scrollTop);
			$("div#chat_board_area").scrollLeft(json.scrollLeft);
		}
...
function responceChatBoardSync(requestUserId){
	...
	var data = {
			command : "INIT_SYNC",
			type : "RESPONSE_CHAT",
			showFlg : $("#chat_board_area").is(':visible'),
			messages: messages,
...
//		mtSessionStorage[sessionStorageKeys[i]]["viewFlg"] = 0;
	...
	let showFlg = $("div#mi_docment_area").is(':visible');
	// 画面共有の下に資料を展開している状態のため、資料情報を相手に送るためにフラグをtrueにする
	if (NEGOTIATION.mIsMyScreenOn && $('#document_material_id').val()) {
		showFlg = true;
	}
