...
const sideBarDisplayCss = {
	'1' : {
		'#doc_scale_zoomin' : {'display' : 'block', 'top' : '150px'},
		'#doc_scale_range' : {'display' : 'block', 'top' : '226px', 'left' : '50px'},
		'#doc_scale_zoomout' : {'display' : 'block', 'top' : '256px'},
		'#doc_scale_value' : {'display' : 'block', 'top' : '286px'},
		'#mi_scroll_arrow_left' : {'top' : '362px', 'left' : '50px'},
		'#mi_scroll_arrow_right' : {'top' : '405px', 'left' : '50px'},
		'.page_content' : {'top' : '329px', 'left' : '50px'},
		'#document_zoom_btn' : {'display' : 'none'},  
		'#document_download' : {'top' : '449px', 'left' : '50px'},// mi_scroll_arrow_rightのheight分 42px + top + margin
		'#img_document_comment' : {'top' : '76px', 'left' : '50px'},
	},
	'2' : {
		...
		'#document_zoom_btn' : {'top' : '120px', 'left' : '50px', 'display' : 'block'},
		'#document_download' : {'top' : '338px', 'left' : '50px'}, 
		'#img_document_comment' : {'top' : '76px', 'left' : '50px'},
	}, 
	'3' : {
		'#doc_scale_zoomin' : {'display' : 'none', 'top' : '204px'},
		...
		'#document_zoom_btn' :  {'top' : '120px', 'left' : '50px', 'display': 'block'},  
		'#document_download' : {'top' : '107px', 'left' : '0px'}, // 2列の左列のため left: 0
		'#img_document_comment' : {'top' : '76px', 'left' : '50px'},
	}
}
...
　/**
	...
	$(document).on('click', '[name=material_select]', function(){
		...
		if(materialsId.length > 0){
			...
			if(materialSelectedListForSetting.length > 0) {
				...
				/*
				//資料共有設定モーダルを開く
				// ECC様(client_id=1821)のみ、アルバイト権限で資料追加を利用可能にするがダウンロード設定はさせない
	...
	$(document).on('click', 'div#document_download', function(){
		...
		// 自分 or 相手に送る　の選択肢を表示
		// MEMO. ダウンロードは資料を提供したユーザだけが実行できる. ソレ以外のユーザにはボタンがない 
		// 持ち主がボタンを押して相手に送るを押した時にダウンロードモーダルが相手に表示される.
		$('#document_download_option').toggle();
	
	});

	/**
	 * 『資料を送る』（ダウンロード）選択肢に応じて、相手にダウンロードさせる（ダウンロードモーダルを表示させる）処理
	 */
	$(document).on('click', '#document_send_download', function(){
	 ...
	 * 『資料を送る』（ダウンロード）選択肢に応じて、相手にダウンロードさせる（ダウンロードモーダルを表示させる）処理
	 */
	$(document).on('click', '#document_own_download', function(){
	 ...
	 */
	 $(document).on('click', '#document_close', function(e) {
		...
		if($("div#mi_docment_area").is(':visible')){
			...
			// SPユーザーに画像の非表示通知を行う
			sendCommand(SEND_TARGET_ALL, {command : "DOCUMENT", type : "SP_CLOSE_DOCUMENT"});
		}
		...
		Object.keys(sideBarDisplayCss[sideBarDisplayStyle]).forEach(function(key) {
			Object.keys(sideBarDisplayCss[sideBarDisplayStyle][key]).forEach(function(elementKey){
				if(elementKey == 'top') {
					...
					if(size > 110) { // MEMO. 110 : ２列になった場合の左列要素のTOPの最大値. これを上回る値で（=１列のときに調整するよう）判定　左列にコメントはなく、左列は調整不要のため。
						size -= paddingSize;
					}
...
function TouchEndEvent(event){
	...
	// 資料読み込んで、サイドバーの調整が終わるまで非表示にしておく
	$('#mi_docment_zoom').css('visibility','hidden');
	// 拡大表示をリセット
	codumentViewState = 1;
	$("li.left_icon_size div#icon-expansion").show();
		...
		setTimeout(function(){
			sideBarUpdate(true);
			$('#mi_docment_zoom').css('visibility','visible'); // サイドバーの調整終了後、表示する
		}, 1500); // 読み込み後の要素有無でスタイルを変えているため、少し遅らせて実行している
	
		// ダウンロード選択肢が開いている場合に閉じる（資料入替時など）
		if ($('#document_download_option').css('display') !== 'none') {
			$('#document_download').trigger('click');
		}
	}
...
function viewDocument(requestUserId, documentImg, canvasImg, canvasImgFlg, DropFirstFlg, MoveDocumentFlg){
	...
	// 画面共有の後ろに資料が展開してある状態のとき、読込時に表示しないようにする。
...
function resizeDocument(DropFirstFlg){
	...
	var compCanvasWidth = $("div#mi_docment_area").width();
	var compCanvasHeight = $("div#mi_docment_area").height();
	if (codumentViewState<=1) {	// 倍率が100%の場合、固定枠と比較する
		compCanvasWidth = orgCanvasWidth;
		compCanvasHeight = orgCanvasHeight;
	}
	// 画像の横幅>固定横幅の場合、固定横幅を設定する
	if (canvasWidth > compCanvasWidth){
		width = compCanvasWidth;
		scroll = "scroll";
	}
	// 画像の縦幅>固定縦幅の場合、固定縦幅を設定する
	if (canvasHeight >compCanvasHeight ) {
		height = compCanvasHeight;
		scroll = "scroll";
	}
	$("div#mi_docment_area").height(height).width(width);
...
function getMousePointTag(drowX, drowY, userId){
	...
	var mouserImg = '<span id="mouse_point_'+userId+'" class="span_emphasis_pointer" style="position:absolute;z-index:26;left:'+drowX+'px;top:'+drowY+'px;"><img src="/img/icon_pointer_pc.svg" class="img_emphasis_pointer" /></span>';
	return mouserImg;
}
...
function isCheckDocumentOwner() {
	...
	// SPEC. ECC様の場合、アルバイト権限でも共有はできるが、ダウンロードボタンは表示はしない.
	check.push(!(clientId === 1821 && staffRole === 'CE_3'));
...
function removeMaterialCommon(keyName, localMaterialId) {
	...
	// 2021.11.11現在では表示中のものは削除できないようになっているため、以下は通らない
...
function showDocumentCommon(documentCanvasLeft, documentUrlFlg, documentUserId, documentUuId, documentMaterialId){
	...
	setTimeout(function(){
		sideBarUpdate(true);
			$('#mi_docment_zoom').css('visibility','visible'); // サイドバーの調整終了後、表示する
			if (NEGOTIATION.mIsScreenOn) { // 初期同期時、読込はするが、サイドバー含め資料を非表示にする
				hideBehindDocument();
			}
	}, 1500); // 読み込み後の要素有無でスタイルを変えているため、少し遅らせて実行している
...
/**
* 他モーダル等の後ろにある資料（非表示状態）を表示する
*/
function showBehindDocument() {
	if ($('#document_material_id').val()) {
		$('#mi_docment_area').css('display', 'block'); // 資料表示
		$('#mi_docment_zoom').css('visibility','visible'); // 資料サイドバー可視化
	}
}

/**
* 他モーダル等の後ろにある資料を非表示にする
*/
function hideBehindDocument() {
	if ($('#document_material_id').val()) {
		$('#mi_docment_area').css('display', 'none'); // 資料非表示
		$('#mi_docment_zoom').css('visibility','hidden'); // 資料サイドバー非表示
	}
}
