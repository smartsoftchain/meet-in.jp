		...
		if ( e.which == 13 ) {	// エンターキー
			...
			if($('#select_sendMode_checkbox').prop('checked')){
				...
				if( $('#chat_board_send_message').val().length ) {	// 入力値あり
					...
					escape(targetId, value, user_name);
...
// サニタイジング済み
function escape(targetId, targetValue, userName) {
	...
	// HTMLサニタイジング（HTML文字列を無害化し、改行コードをbrタグに置き換え）
		...
		//サニタイズしたtextValueを改行位置で配列にして<div>でひとつずつ囲む
		...
		splitTextArray.forEach(function(splitText){
			...
			displayTextContents += '<div class="chat_board_message_text_value"' + 'data-user-name="'+userName+'">' + splitText + '</div>';
		});
