...
//		var len = showVideoArray.length;
		...
		let horizontal_height = $("#video_target_video_area_horizontalbox").height();
		if( tmpW >= tmpH * (tmpRate) ){
			$fitArea.width(tmpH * (tmpRate));
			if ($("#mi_docment_area").is(':visible')) {
				tmpH = tmpH - horizontal_height;
				$fitArea.height(tmpH);
				$fitArea.css('margin-top', horizontal_height + 'px');
			} else {
				$fitArea.height(tmpH);
				$fitArea.css('margin-top', '0px');
			}
		}else{
			console.debug(tmpRate + " " + tmpW +" " + tmpH);
			$fitArea.width(tmpW);
			if ($("#mi_docment_area").is(':visible')) {
				let height = tmpW / (tmpRate);
				height = height - horizontal_height;
				$fitArea.height(height);
				$fitArea.css('margin-top', horizontal_height + 'px');
			} else {
