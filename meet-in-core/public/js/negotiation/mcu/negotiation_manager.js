...
(function (NegotiationManager) {
	...
	var NegotiationMain = (function () {
		...
		// 再接続
		var reconnect = function(successCb, errorCb
			) {
			if (mMeetinWebSocketProc) {
				mMeetinWebSocketProc.destroy();
			}
...
*/
				reconnectRoom(successCb, errorCb);
			} else if (mMeetinWebSocketProc) {
		...
		// 再接続
		NegotiationMain.prototype.reconnect = function(successCb, errorCb
			) {
			reconnect(successCb, errorCb);
		};
		...
		var cameraStreamDisable = function(
				...
				if (mMeetinCore) {
					mMeetinCore.cameraStreamDisable(
					);
				}
		};
		NegotiationMain.prototype.cameraStreamDisable = function(
		) {
			cameraStreamDisable();
		};
		...
		var cameraStreamEnable = function(
				...
				if (mMeetinCore) {
					mMeetinCore.cameraStreamEnable(
					);
				}
		};
		NegotiationMain.prototype.cameraStreamEnable = function(
		) {
			cameraStreamEnable();
		};

		// メディアストリームをチェンジ
		var cameraChromakeyOn = function(
				stream,
				completeCallback
		) {
				console.log('cameraChromakeyOn');
				if (mMeetinCore) {
					mMeetinCore.replaceAndSendVideoStreamToAllTarget(
						null,
						{},
						stream,
						null,
		...
		NegotiationMain.prototype.cameraChromakeyOn = function(
			stream
			) {
			cameraChromakeyOn(
				stream,
		...
		var cameraChromakeyOff = function(
			completeCallback
			) {
				console.log('cameraChromakeyOff');
			if (mMeetinCore) {
				mMeetinCore.replaceAndSendVideoStreamToAllTarget(
					null,
					{},
		...
		NegotiationMain.prototype.cameraChromakeyOff = function(
			) {
			cameraChromakeyOff(
				null
			);
