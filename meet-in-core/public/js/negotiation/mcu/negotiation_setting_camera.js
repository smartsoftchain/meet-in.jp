...
const initDevice = function (camera_id, audio_id){
	new Promise(function(resolve, reject) {
		...
		if (USER_PARAM_BROWSER != 'Edge') {
			...
			if (localVideoResolution == '0') { localVideoResolution = "idealQvga"; }
			else if (localVideoResolution == '1') { localVideoResolution = "idealVga"; }
			else if (localVideoResolution == '2') { localVideoResolution = "idealSvga"; }
			else if (localVideoResolution == '3') { localVideoResolution = "idealHd"; }
			else if (localVideoResolution == '4') { localVideoResolution = "idealFull-hd"; }
...
console.log("カメラIDが切り替:("+ NEGOTIATION.use_camera_facingMode +")");
	...
	if (NEGOTIATION.isMyCameraOn || NEGOTIATION.isMyMicOn) {
...
//	}
	...
	// フレームレート(ビデオ設定)
	setVideoIntervalTime(NEGOTIATION.use_fps);
...
function getCameraConstraints() {
	...
	if( USER_PARAM_IS_MOBILE || USER_PARAM_IS_IPAD_PC) {
		...
		if (NEGOTIATION.use_camera_id && NEGOTIATION.use_camera_id != 'null') {
			// カメラID指定
			if (NEGOTIATION.use_camera_id != 'default') {
...
function getMicConstraints() {
	NEGOTIATION.use_mic_id = localStorage.getItem('use_mic_id');
	if (NEGOTIATION.use_mic_id == null || NEGOTIATION.use_mic_id == "null" || NEGOTIATION.use_mic_id == "default") {
		var constraints = {
			audio: true,
		};
		return constraints;
...
function getMediaConstraints() {
	...
	videoConfig = getCameraConstraints();
	audioConfig = getMicConstraints();
	config.video = videoConfig.video;
...
function openVdoQ() {
	// 別タブの設定
	var subp = "https://manual.meet-in.jp/?p=676";   // 表示するページ(URL)
	var subn = "_blank";   // 別タブ指定

	// 別タブで表示
	window_q = window.open(subp, subn);
}
...
function openMicQ() {
	// 別タブの設定
	var subp = "https://manual.meet-in.jp/?p=679";   // 表示するページ(URL)
	var subn = "_blank";   // 別タブ指定

	// 別タブで表示
	window_q = window.open(subp, subn);
	// window_micq = window.open("https://manual.meet-in.jp/?p=679, "_blank");
}
