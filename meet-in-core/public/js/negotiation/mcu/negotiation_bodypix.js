...
//console.log(`enabled_video : ${enabled_video} : enabled_audio : ${enabled_audio}`);
console.log(`${localVideo.videoWidth}x${localVideo.videoHeight}`);
		localVideo.width = videoWidth;
		localVideo.height = videoHeight;
		bodyPixCanvas.width = videoWidth;
		bodyPixCanvas.height = videoHeight;
		canvasBg.width = videoWidth;
		canvasBg.height = videoHeight;
		canvasVideo.width = videoWidth;
		canvasVideo.height = videoHeight;
		ctxVideo.width = videoWidth;
		ctxVideo.height = videoHeight;
...
const updateSegment = async () => {
	...
	ctxVideo.drawImage(localVideo, 0, 0, localVideo.videoWidth, localVideo.videoHeight, 0, 0, ctxVideo.width, ctxVideo.height);
	...
	.then(segmentation => {
		...
		if (checkboxBokehEffect == true) {
			...
			bodyPix.drawBokehEffect(bodyPixCanvas, canvasVideo, segmentation, backgroundBlurAmount, edgeBlurAmount, flipHorizontal);
		} else {
...
//  const maskBlurAmount = 3;
	...
	if (checkboxBokehEffect == true) {
		...
		if (checkboxChromakey == true) {
			bodyPix.drawMask(canvasTmp, canvasVideo, bodyPixMask, opacity, maskBlurAmount, flipHorizontal);
			chromaKey(ctxTmp, canvasTmp, ctxBg, canvasBg, bodyPixCtx, bodyPixCanvas);
		} else {
			bodyPix.drawMask(bodyPixCanvas, canvasVideo, bodyPixMask, opacity, maskBlurAmount, flipHorizontal);
		}
	}
}
...
const onOkBodypixDialog = () => {
	if (NEGOTIATION.isMyCameraOn == false) {
		$.when($('#button_camera').trigger('click')).done(() => {
			// カメラONにしてストリーム配信が完了するまでまつ
			let publishingTimer = setInterval(() => {
				if (isCameraOnPublished == true) {
