...
$(function () {
		...
		$.ajax({
			...
			success: function(resultJson) {
				...
				for(var i = 0; i < result.length; i++){
					...
					result[i]["download_flg"] = Number(result[i]["download_flg"]);
					// material_typeの値をここで判別するkeyを追加する
					if(result[i]["material_type"] == 1) {
							result[i]["type_1"] = 1;
					}else if(result[i]["material_type"] == 2) {
							result[i]["type_2"] = 2;
					} else if(result[i]["material_type"] == 3) {
							result[i]["type_3"] = 3;
					}
	 ...
	 */
	 $(document).on('click', '#document_close', function() {
...
// 2018/04/20 太田
			...
			loadDocument(NO_NOTIFICATION, 1, 1);
			// MCU メイン映像領域非表示
//			if (!$("#video_target_video_area_horizontalbox").is(':visible')) {
...
// 	console.log("top=("+ parseInt($("div#mi_docment_area").css('top')) +")")
	if (DropFirstFlg==1) {	// 資料初期表示のみ(サムネイルドロップ時)
		...
		var area_top = parseInt($('div#fit_rate').css('top'));
...
//console.log("幅オーバー")
			var left = ($("#fit_rate").width() - $("div#mi_docment_area").width()) / 2;
 ...
 * @returns
 */
function resizeCanvas(requestUserId, DropFirstFlg = 0){
	// 現在のキャンバスデータを取得する
...
function hideDocument(){
	...
	if($("div#mi_docment_area").is(':visible')){
		...
		// 資料の領域を非表示にする
		$("div#mi_docment_area").hide();
		// fit_rateのmargin-topを0にする
		$("#fit_rate").css('margin-top', '0px');
...
function showMoveThumbnailIcon(thumbnailCount){
	...
	// 1200以上1260以下、資料3枚以上
	if(clirntWidth > 1200 && clirntWidth <= 1260){
		if(thumbnailCount>=3){
			arrowMaxWrap('200px','150px');
			thumbnailCount > 3 ? showThumbnailArrow(): hideThumbnailArrow();
	...
	// 1260以上1300以下、資料４枚以下
	if(clirntWidth > 1260 && clirntWidth <= 1300){
		if(thumbnailCount>4){
	...
	// 1300以上1500以下、資料４枚以上
	if(clirntWidth > 1300 && clirntWidth <= 1500){
		if(thumbnailCount>4){
			arrowMaxWrap('250px','200px')
			showThumbnailArrow();
		}else if(thumbnailCount==4){
...
function hideMoveThumbnailIcon(thumbnailviewCount){
	...
	if(thumbnailviewCount!==0){
		...
		// 1200以上1300以下、資料3枚以下の場合矢印非表示
		if(clirntWidth > 1200 && clirntWidth <= 1300 && thumbnailviewCount<=3){
			hideThumbnailArrow();
		}

		// 1300以上、資料4枚以下の場合矢印非表示
		if(clirntWidth>1300&&thumbnailviewCount<=4){
			hideThumbnailArrow();
		}
	}
}
 ...
 */
function documentResizeEvent(DropFirstFlg = 0){
	// 資料が表示されている場合のみリサイズ処理を実行する
	if($("div#mi_docment_area").is(':visible') && currentDocumentId != 0){
		// 資料表示領域を設定する
		setOrgCanvasSize();
		// キャンバスのリサイズ処理
		resizeCanvas(NO_NOTIFICATION, DropFirstFlg);
	}
}
...
//		publishHorizontalVideo();
	...
	// 拡大表示をリセット
	codumentViewState = 1;	// [1:縮小, 1.7:拡大1, 2.4:拡大2, 3.1:拡大3]
	console.log("codumentViewState=("+codumentViewState+")")
	$("li.left_icon_size div#icon-expansion").show();
	$("li.left_icon_size div#icon-reduction").hide();
...
$(function() {
	$(window).on('load resize', function() {
		...
		// 1200以上1300以下
		if(clirntWidth > 1200&&clirntWidth <= 1320){
		...
		// 1300以上1500以下
		}else if(clirntWidth <= 1500&&clirntWidth > 1320){
	...
	if( sessionStorageKeys.length > 0 ) {
		...
		$(window).resize(function(){
			...
			if(documentCurrentCount){
				...
				if(clirntWidth > 1200&&clirntWidth <= 1300){
					buttonSizeContorol('130px','110px')
				...
				// それ以上
					...
					buttonSizeContorol('145px','110px')
					if(documentCurrentCount>=5){
						showThumbnailArrow();
					}else {
						hideThumbnailArrow();
