...
const McuRest = class {
  ...
  /**
   * @function statusVideoUpdate
   * @desc 指定したストリームの状態を更新
   * @param {String} roomId       -ルームID
   * @param {String} streamId     -停止させるストリームのID
   * @param {String} value        - "active" | "inactive"
   * @param {Function} callback   -コールバック
   */
   statusVideoUpdate(roomId, streamId, value, callback) {
    let body = [
      {
        op: 'replace',
  ...
  /**
   * @function doropStream
   * @desc 指定された部屋から指定されたストリームを削除します
   * @param {String} roomId       -ルームID
   * @param {String} streamId     -削除させるストリームのID
   * @param {Function} callback   -コールバック
   */
   doropStream(roomId, streamId, callback) {
    this.send(
      'DELETE',
      `/rooms/${roomId}/streams/${streamId}`,
      this.UNUSED_BODY,
