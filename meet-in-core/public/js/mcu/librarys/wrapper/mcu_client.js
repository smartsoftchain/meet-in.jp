...
const McuClient = class {
  ...
  leave() {
	...
	//
	publicationInfo.publication.stop();
  ...
  _publishCore(stream, label, view, options, succeededCallback, failedCallback) {
    ...
    // 公開
// console.log('_publishCore publish +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++');
// console.log('label=['+label+'] view=['+view+']');
// console.log(stream);
...
// console.log('_publishCore addStream START +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++');
      this.rest.addStream(this.roomId, publication.id, view, () => {
		console.log('[McuClient] add stream succeeded [' + publication.id +']');
// console.log('_publishCore addStream END +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++');
   ...
   * @function addPublish
   * @desc ストリーム公開
   * @param {String} publicationId          -PublishID
   * @param {String} view                   -ビュー
   * @param {Function} succeededCallback    -成功コールバック
   */
   addPublish(publicationId, view, succeededCallback) {
    console.log('[McuClient] addPublish publicationId=['+ publicationId +']');

	// 公開
    // ルームの公開ストリームに自分のストリームを追加
    this.rest.addStream(this.roomId, publicationId, view, () => {
      console.log('[McuClient] addStream stream succeeded');
	  // 通知
      if (succeededCallback != null) {
        succeededCallback();
      }
   ...
   * @param {String} view   -ビュー
   */
   removePublish(publicationId, view) {
    console.log('[McuClient] removePublish');
   ...
   * @param {Function} succeededCallback    -成功コールバック
   */
   statusUpdate(publicationId, value, succeededCallback) {
    console.log('[McuClient] statusUpdate publicationId=['+ publicationId +'] value=['+ value +']');

	  // 公開
      // 状態変更
      this.rest.statusVideoUpdate(this.roomId, publicationId, value, () => {
        console.log('[McuClient] statusVideoUpdate stream succeeded');
		// 通知
        if (succeededCallback != null) {
          succeededCallback();
        }
  ...
  /**
   * @function doropPublish
   * @desc 公開中のストリーム削除
   * @param {String} publicationId          -PublishID
   */
   doropPublish(label, view) {
    console.log('[McuClient] doropPublish');

	// 停止する公開情報を探す
    let publicationInfo = this._findPublishing(label, view);
    if (publicationInfo == null) {
      console.error(`[McuClient] doropPublish error: not found publication label =${label}, view = ${view}`);
      return;
    }
// console.log('+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++');
// console.log(publicationInfo);
    ...
    // ストリーム停止
	if( publicationInfo.stream != null ) {
//console.log(publicationInfo.stream.mediaStream);
		const tracks= publicationInfo.stream.mediaStream.getTracks();
		tracks.forEach(function(track) {
		  track.enabled = false;		// VideoトラックOFF
		  track.stop();
		});

		if( publicationInfo.stream.mediaStream.getVideoTracks().length != 0 ) {
// console.log('Video件数=('+ publicationInfo.stream.mediaStream.getVideoTracks().length +')');
			publicationInfo.stream.mediaStream.removeTrack(publicationInfo.stream.mediaStream.getVideoTracks()[0]);
		}
		if( publicationInfo.stream.mediaStream.getAudioTracks().length != 0 ) {
// console.log('Video件数=('+ publicationInfo.stream.mediaStream.getAudioTracks().length +')');
			publicationInfo.stream.mediaStream.removeTrack(publicationInfo.stream.mediaStream.getAudioTracks()[0]);
		}

		// publicationInfo.stream.mediaStream=null;
	}

    // // ルームの映像からストリームを除外
    // this.rest.doropStream(
    //   this.roomId,
  ...
  async stopPublish(label, view) {
    ...
    // 停止する公開情報を探す
    let publicationInfo = await this._findPublishing(label, view);
    if (publicationInfo == null) {
      console.warn(`[McuClient] stopPublish error: not found publication label =${label}, view = ${view}`);
      return;
    }

	// ルームの映像からストリームを除外
    await this.rest.removeStream(
      this.roomId,
      publicationInfo.publication.id,
...
// 	}
    ...
    // 一覧から削除
    this.publicationInfos = await this.publicationInfos.filter(publicationInfo => publicationInfo.label != label || publicationInfo.view != view);
  }
  ...
  /**
   * @function _findPublishing2
   * @desc 公開情報を検索する
   * @param {String} label  -ストリームのラベル
   * @param {String} view   -ビュー
   * @return {Object}       -発見した公開情報（見つからなかった場合はnull）
   */
   _findPublishing2(label, view, flgOnOff) {
    let publicationInfos = this.publicationInfos.filter(publicationInfo => publicationInfo.label == label && publicationInfo.view == view && publicationInfo.flgOnOff == flgOnOff);
    if (publicationInfos.length == 0) {
      return null;
    }
    return publicationInfos[0];
