...
$(function () {
	...
	// SPEC. ゲストは 保存先がないことから 画像追加、画像削除を許可できない.
	let staffData = {
		"staff_type" : $('input[id=staff_type]').val(),
		"staff_id"   : $('input[id=staff_id]').val()
	};
	if(staffData.staff_type == 'ZZ' || staffData.staff_type == '') {
		$("#btn_add_background_image").hide();
	}
...
function changeGuestBodyPix(imagePath){
	...
	if (USER_PARAM_BROWSER == "Chrome") {
		...
		// safariを利用する際にはbodypixを使用する可能性を考慮しコメントアウト
