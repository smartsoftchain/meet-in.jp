...
/*		
		...
		var sendBinary = function(
			...
			mMeetinWebSocket.sendBinary(binary);
			return true;
		};

		// バイナリ送信
		MeetinWebSocketProc.prototype.sendBinary = function(
				binary
			) {
			sendBinary(
				binary
...
//			meetinFlashTargetVideo_onHangup(target_user_id);
...
//			var targetWebSocketPeerId = json.from_peer_id;
//			if (16 == targetWebSocketPeerId.length) {
//				targetWebSocketPeerId = "S" + targetWebSocketPeerId;
//			}

//			meetinFlashTargetVideo_callTarget(targetWebSocketPeerId, json.from_user_id, mUserId);
...
//			sendEnterRoomReceived(json.from_peer_id);
		...
		var runEnterRoomReceived = function(
			...
			// 相手のメディアストリーム（映像）を要求する
//			sendRequestVideoStream(i, targetPeerId, null);
			
			// 自分のメディアストリーム（マイク）を送る
//			runRequestAudioStreamSub(null);

			// 相手のメディアストリーム（マイク）を要求する
//			sendRequestAudioStream(i, targetPeerId, null);
			
			// 自分のメディアストリーム（画面共有）を送る
//			runRequestScreenStreamSub(null);

			// 相手のメディアストリーム（画面共有）を要求する
//			sendRequestScreenStream(i, targetPeerId, null);
