...
$(function () {
	...
	$(document).on('touchmove', 'canvas#sp_img_canvas', function(e){
		...
		if(docOnMouseFlg && (selectLeftTool == TOOL_PEN_NORMAL || selectLeftTool == TOOL_PEN_HIGHLIGHT || selectLeftTool == TOOL_PEN_ERASER)){
			...
			if (displayImgScale > 1) {
				// 拡大していた場合、その拡大率で割る
				touchX = e.originalEvent.touches[0].pageX / displayImgScale;
				touchY = e.originalEvent.touches[0].pageY / displayImgScale;

				positionX = positionX / displayImgScale;
				positionY = positionY / displayImgScale;
			}
			// 要素の左上からの距離を計算
			var drowX = touchX - positionX ;
			var drowY = touchY - positionY ;
		...
		if (e.originalEvent.touches.length >= 2) {
			// ２本指以上の場合は, ピンチイン、アウトできる状態にする
			zoomInAndOutDocument();
		} else {
			// 1本指の場合はスクロールできる状態にする
	...
	$(document).on('touchstart', '#sp_img_canvas', function(e){
		...
		// 拡大していた場合は、その拡大率で割る
		if (displayImgScale > 1 && selectLeftTool !== TOOL_CURSORE_SELECT) {
			onMouseX = onMouseX / displayImgScale;
			onMouseY = onMouseY / displayImgScale;
		}
...
function loadDocument(requestUserId, DropFirstFlg){
	...
	if(mtSessionStorage && keyName in mtSessionStorage && currentDocumentId != 0){
		...
		loadImages(imagePaths, loadedImgsInfo, function(loadedImgsInfo) {
			...
			loadImageSizePromise($("#sp_img_document").attr("src"))
			.then(res => {
				...
				// テキスト入力オブジェクトの設置.
				setDocumentTextInputData();

				initScale(); // 倍率を初期化する

				if (selectLeftTool === TOOL_CURSORE_SELECT) {
					no_scaling(); // デフォルトのピンチイン動作等を止める
				}
...
function setDocumentTextInputData() {

	$("#document_text_field").css({'width':$("#sp_img_document").css('width'), 'height': $("#sp_img_document").css('height')});
...
$(window).on("orientationchange resize", function(){
	...
	if(0 < $('.document_text').length) {
		...
		loadImageSizePromise($("#sp_img_document").attr("src"))
		.then(res => {
			$('#sp_img_canvas').css({'width':$("#sp_img_document").css('width'), 'height': $("#sp_img_document").css('height')});
			// テキスト入力オブジェクトの設置.
			setDocumentTextInputData();
		});
	}
...
function setOrgCanvasSize(){
	orgCanvasWidth = $("#sp_img_document_area").width();
	orgCanvasHeight = $("#sp_img_document_area").height();
}
...
function getProportion(){
	...
	// 比率の計算を行う
	if(!canvasHeight){
		canvasHeight = $('#sp_img_document').height();
	}
...
function hideDocumentCommon(){
	...
	// 画面共有の下に資料展開している場合は、再表示するため初期化しない
...
function getMousePointTag(drowX, drowY, userId){
	let iconPointerClassName = "icon-pointer-sp-phone";
	var ua = navigator.userAgent.toLowerCase();
	const isAndroidTablet = ua.indexOf("android") !== -1 && 'ontouchend' in document && window.outerWidth > 800;

	// デバイスがipadか判定 画面幅によって、ipad pro等大きいサイズのものとそれ以外用に調整する
	const isIPad = /iPad|Macintosh/i.test(navigator.userAgent) && 'ontouchend' in document
	if (isIPad || isAndroidTablet) {
		iconPointerClassName = "icon-pointer-sp";

		if (Math.abs(window.orientation) === 90) {
			setTimeout(function() {
				if (window.outerWidth < 1200) {
					$('.' + iconPointerClassName).css({
						"width": "26px",
						"height": "26px"
					});  // iPad横用
				} else {
	...
	// 強調ポインター
	var mouserImg = '<span id="mouse_point_'+userId+'" class="'+iconPointerClassName+'" style="left:'+drowX+'px;top:'+drowY+'px;"></span>';
	return mouserImg;
}

// iPad　縦横切り替え時にiconサイズを変える
$(window).bind('orientationchange', function() {
	const iconElem = $('.icon-pointer-sp');
	console.log(iconElem.length > 0);
...
function receiveDocumentJson(json){
		...
		if($("div#sp_document_area").is(':visible') && currentDocumentId != 0){
			...
			for (var i = 0; i < json.position.length; i++)
			{
				...
				if(json.position[i].writeLineFlg){
					if(json.position[i].selectLeftTool === 6){
						return;
					}
					// 線を描いた人の比率と自分の比率を考慮し計算を行う
	...
	} else if (json.type == "SP_CLOSE_DOCUMENT"){ // PC用のコマンドをSP用にした
			// 自分の画像を閉じる
			hidePhotoImg();
	}else if(json.type == "CLEAR_CURSORE_EMPHASIS_POINTER"){

		// 送信者の強調ポインターが画面に残らないように削除する.
		let mouse_point = $('#mouse_point_'+json.userId);
		if(0 < mouse_point.length) {
			mouse_point.remove();
		}
	}
}
...
function showDocumentCommon(documentCanvasLeft, documentUrlFlg, documentUserId, documentUuId, documentMaterialId){
	...
	if (NEGOTIATION.mIsScreenOn) { // 画面共有されている状態で初期同期時、読込はするが、資料を非表示にする
		hideBehindDocument();
	}

	$("#header_video_reset").trigger('click'); // レイアウトを元に戻す. SPにはない要素 2021.11.19現在
}
...
function resetLayout(){
	...
	// 資料のリセット
	if($("div#mi_docment_area").is(':visible') && currentDocumentId != 0){ // mi_docment_areaはSPにない要素
		...
		// アイコンなどの表示位置をリセットする　// アイコンも SPにはまだない 2021.11.19現在
		documentResizeEvent();
	}
}
/**
...
/**
* 他モーダル等の裏にある資料（非表示状態）を表示する
* MEMO. 今後もしSPにも資料サイドバー等が実装された際には、PC版と同様に要素を隠す必要あり。
*/
function showBehindDocument() {
	if ($('#document_material_id').val()) {
		$('#sp_document_area').css('display', 'block'); 
		$('#sp_document_area').css('visibility', 'visible');
	}
}

function hideBehindDocument() {
	if ($('#document_material_id').val()) {
		$('#sp_document_area').css('display', 'none');
		$('#sp_document_area').css('visibility', 'hidden');
	}
}


/*
*	SP用のピンチイン（拡大）、ピンチアウト（縮小）処理
*/
...
var imgScale = 1.0; // ピンチ量によって、可変するscale値
var displayImgScale = 1.0; // 実際に画面に反映するscale値

/**
 * ピンチ量によって、scaleを変更する処理 
 */
function transScale(e) {
	e.preventDefault();

	const touches = e.touches;

	if (touches.length > 1 && selectLeftTool == TOOL_CURSORE_SELECT) {
		...
		var pinchDistance = Math.sqrt( Math.pow( x2-x1, 2 ) + Math.pow( y2-y1, 2 ));

		if (baseDistance && baseImageWidth && baseImageHeight) {
			imgScale = pinchDistance / baseDistance;
			
			if (1 <= imgScale && imgScale <= 3) {
				
				const diffImageScale = displayImgScale - imgScale > 0 ? displayImgScale - imgScale : imgScale - displayImgScale;

				if (diffImageScale <= 0.5) { // 差が大きいものは急拡大／縮小になるため弾く

					displayImgScale = imgScale;

					const scale = "scale(" + displayImgScale +","+ displayImgScale +")";
				...
				$('.sp_img_document_wrapper').removeClass('hide_scroll');　// scrollbar 非表示　解除
			} else {
				if (imgScale < 1.0) { // 最小倍率に至った場合はscrollbarを隠す
					$('.sp_img_document_wrapper').addClass('hide_scroll');
				}
			}
		} else {
			baseDistance = pinchDistance;
			baseImageWidth = $('#sp_img_canvas').width();
			baseImageHeight = $('#sp_img_canvas').height();
		}
 ...
 */
function initScale() {
	$('#sp_img_canvas').css('transform', '');
	$('#sp_img_document').css('transform', '');
	$('#document_text_field').css('transform', '');

	displayImgScale = 1.0 // 変数の倍率値も初期化する
}
...
function zoomInAndOutDocument() {
	document.addEventListener("touchmove", transScale, { passive: false });
}

function returnZoomInAndOutDocument() {
	document.removeEventListener("touchmove", transScale, { passive: false });
}
