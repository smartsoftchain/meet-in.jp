...
function procScreenIsOn(json) {
	...
	hideBehindDocument(); // 資料が表示されていたら、非表示にする
...
function procScreenIsOff(json) {
	...
	let lastVisitorsNumber = 0;

	// 最後の入室者のidを取得する
	for(var i=0; i<MEETIN_MAIN_MAX_PEOPLE; i++){
		const targetVideo = $("#negotiation_target_video_"+i);
		if (targetVideo.is(':visible')) {
			lastVisitorsNumber = targetVideo.data("id");
		}
	}
	
	$('#negotiation_share_screen').removeClass("big_video_area");
	if (lastVisitorsNumber > 2) { // 3人以上の場合のvideo調整 最後の入室者を大きいvideoに設定 
		$('#negotiation_target_video_' + lastVisitorsNumber).addClass("big_video_area");
		var bigVideoId = $(".big_video_area").data("id");
		// 元のレイアウトに戻す
		spChangeLayout(bigVideoId, "#2");
	}　else if ($('#user_id').val() == 1) {　
		spChangeLayout(2, "#2"); // 2人のときのレイアウト　元の状態に戻す SP側がホストの場合
	} 
	else {
		spChangeLayout(1, "#2"); // 2人のときのレイアウト　元の状態に戻す SP側がゲストの場合
	}
