...
if (
  ...
  __CHROME_EXT_SCREENRECORDER__ = {
    name: "Meetin録画",
    url:
      "https://chrome.google.com/webstore/detail/meetin%E9%8C%B2%E7%94%BB/mmjgmghpaeanjmgofpmjfpfacohimpaf?hl=ja",
  };
  __CHROME_EXT_SCREENSHARE__ = {
    name: "Meetin画面共有",
    url:
      "https://chrome.google.com/webstore/detail/meetin画面共有/pfkkknnhliihpaolobjbnkpdkhcfneio",
  };
  ...
  __CHROME_EXT_SCREENRECORDER__ = {
    name: "Meetin録画",
    url:
      "https://chrome.google.com/webstore/detail/meetin%E9%8C%B2%E7%94%BB/feadpakpdafikklbhmngppnagacgeene?hl=ja",
  };
  __CHROME_EXT_SCREENSHARE__ = {
    name: "Meetin画面共有",
    url:
...
// 本番環境にGAトラッキングコードを埋め込む
$(function () {
  var host = location.hostname;
  if (host == "meet-in.jp") {
    $("head").append(
      '<!-- Global site tag (gtag.js) - Google Analytics --><script async src="https://www.googletagmanager.com/gtag/js?id=UA-132461738-2"></script><script>window.dataLayer = window.dataLayer || [];  function gtag(){dataLayer.push(arguments);}  gtag("js", new Date());  gtag("config", "UA-132461738-2");</script>'
    );
  }
});
 ...
 */
function getFileNamesRemoveExtensions (fileName) {
  var lastDotIndex = fileName.lastIndexOf(".");
  if (lastDotIndex > -1) {
    var fileName = fileName.substring(0, lastDotIndex);
  }
  return fileName
}

/**
 * テキストボックスの値の半角英字を全角に変換する処理
 */
$(function () {
