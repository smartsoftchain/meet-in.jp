...
function isChromeBook(ua_org) {
	let os = ua_org.toLowerCase();
	
	return os.indexOf('cros') !== -1;
}
...
/**
 * 要素の長辺を判定する。
 * @param targetWidth 判定したい要素のwidth
 * @param targetHeight 判定したい要素のheight
 * @return true: widthが長辺, false: heightが長辺
 */
function isHorizontalElement(targetWidth, targetHeight) {
	if (Math.max(targetWidth, targetHeight) == targetWidth) {
		return true;
	} else {
		return false;
	}
