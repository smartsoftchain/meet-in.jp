...
//define('SERVICE_MODE', "production");
...
function getCurrentDbObject(){
	$config  = new Zend_Config_Ini(APP_DIR . 'configs/system.ini', SERVICE_MODE);
	$dsn  = "mysql:dbname=tmo_20160927;host=localhost;";
	$user = "root";
...
function debugMemory(){
	$memoryMax = memory_get_usage();
	$memoryUse = memory_get_usage(true);
	error_log("memory:{$memoryUse} / {$memoryMax}");
}
...
function getCompanyDict($row, $splitPatterns){
	...
	$companyDict["url"] 					= !is_null($row["url"]) ? $commonDao->escape($row["url"]) : "";
	if(mb_strlen($companyDict["url"]) > 128){
		// urlは1000から128に変更な為、128を超えている場合は切り取る
		$companyDict["url"] = mb_substr($companyDict["url"], 0, 128);
	}
...
function getApproachTargetDict($row, $companyDict){
	...
	$approachTargetDict["mail"] 						= !is_null($row["email"]) ? $commonDao->escape($row["email"]) : "";
	$approachTargetDict["postcode"] 					= !is_null($row["mail"]) ? Application_CommonUtil::getOnlyNumbers($commonDao->escape($row["mail"])) : "";
	...
	$approachTargetDict["remarks"]						= !is_null($row["consumer_content"]) ? $commonDao->escape($row["consumer_content"]) : "";
 ...
 */
function getApproachTargetHistorySql($oldApproachTargetDict, $newApproachTargetDict, $clientDbHistoryDao, $migrationDao){
	// 戻り値
	$result = "";
	// 履歴を作成する為に必要な情報を設定する
	...
	if(!$approachTargetHistory){
		// 履歴が１件も存在しない場合、１件目の履歴と次に変更する履歴を作成する
		$result = $migrationDao->migrationCreateClientDbApproachTargetHistoryInsertSql($oldApproachTargetDict);
		$result .= $migrationDao->migrationCreateClientDbApproachTargetHistoryInsertSql($newApproachTargetDict);
	}else{
		// 既に存在する場合、今回変更する履歴のみ作成する
		$result = $migrationDao->migrationCreateClientDbApproachTargetHistoryInsertSql($newApproachTargetDict);
	}
 ...
 */
function registOrSqlStrByApproachTarget($newDb, $newApproachTargetDict, $oldApproachTargetDict, $tableName, $migrationDao){
	...
	if($resultApproachTargeId){
		// 登録処理
		$migrationDao->migrationClientDbApproachTargetRegist($newApproachTargetDict);
	}else{
		// 更新SQLを作成
		$clientDbApproachTargetSql = $migrationDao->createClientDbApproachTargetUpdateSql($newApproachTargetDict, $approachTargetId);
	}
...
function execFileInsertQuery($newClientId){
	...
	debugMeg("exec file insert db:{$config->datasource->params->dbname}");
 ...
 * @return multitype:unknown
 */
function moveFileInsertQuery($newClientId){
	// コマンド実行でファイル登録
	$config = Zend_Registry::get('config');
	$dirPath = $config->file->migration->path;
	$oldFileName = "{$newClientId}_{$config->file->migration->name}";
	$newFileName = date("YmdHis")."_{$newClientId}_{$config->file->migration->name}";
	$oldFullPath = "{$dirPath}/{$oldFileName}";
	$newFullPath = "{$dirPath}/{$newFileName}";
	$cmd = "mv {$oldFullPath} {$newFullPath}";
	exec($cmd, $output, $return);
