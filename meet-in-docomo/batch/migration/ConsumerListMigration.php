...
try{
	...
	try{
		...
		foreach($consumerList as $consumer){
			...
			try{
				// 重複チェックを行いつつデータ移行を行う
				checkOverlapCompanyAndRegist($newDb, $newClientId, $approachListId, $clientDbRelationId, $companyDict, $approachTargetDict, $fp);
			}catch (Exception $e){
 ...
 */
function checkOverlapCompanyAndRegist($newDb, $newClientId, $approachListId, $clientDbRelationId, $companyDict, $approachTargetDict, $fp){
	...
	if($approachTarget){
		...
		if($approachTarget){
			...
			if($approachTarget["converted_base_name"] == $convertedBaseName){
				...
				// 同じファイル内で直接INSERT後にファイルでUPDATEするとエラーになるので処理を分岐させる
				$clientDbCompanySql = registOrSqlStrByCompany($newDb, $companyDict, $company, "migration_consumer_company_id", $clientDbCompanyDao);debugMeg("2");
				debugMeg("26");
				if($clientDbCompanySql != ""){
					fwrite($fp, $clientDbCompanySql);
					//$newDb->query($clientDbCompanySql);
