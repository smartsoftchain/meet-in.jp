...
class MailModel extends AbstractModel{
	 ...
	 */
	Public  function sendRegistAaAccountMail($action, $staffDict){
		...
		$subject =  "【ビジネスdXストア　dXオンライン営業】アカウント登録情報のお知らせ";
		...
		$message .= "\r\n";
		$message .= $this->partsContactInformationMeetIn();
		// $message .= "Webフォン：\r\n";
	...
	public function sendRegistClientMail($client, $stafflist, $categoryList, $serviceList){
		...
		$subject  = "【ビジネスdXストア　dXオンライン営業】会社情報登録のお知らせ";
	...
	/**
	 * CEアカウントを新規・編集した場合のメール送信
	 *
	 * @param unknown $action 文字列 登録/編集
	 * @param unknown $staffDict
	 * @param unknown $client
	 */
	Public  function sendRegistCeAccountMail($action, $staffDict, $client){
		// 権限の文言取得
		...
		if ($action == '編集') {
			...
			$message  = "----------------------------------------------------------------- \r\n";
			$message .= "NTTドコモ　ビジネスdXストアをご利用いただきまして、誠にありがとうございます。 \r\n";
			$message .= "dXオンライン営業にご登録した情報の" . $action . "がありましたのでご連絡申し上げます。 \r\n";
			$message .= "※本メールは、お申込みいただきましたdXオンライン営業を提供する株式会社アイドマ・ホールディングスより送信しております。 \r\n";
			$message .= "\r\n";
			$message .= "----------------------------------------------------------------- \r\n";
			$message .= "サービス名：：\r\n";
			$message .= "　dXオンライン営業\r\n";
			$message .= "\r\n";
			$message .= "ログインURL：\r\n";
			$message .= "https://docomodx.aidma-hd.jp　\r\n";
			$message .= "※本URLより、ビジネスｄアカウントでログインして下さい。　\r\n";
			$message .= "\r\n";
			...
			// 権限ごとに件名変更
			$subject =  "【ビジネスdXストア　dXオンライン営業】サービス利用準備完了のお知らせ";
			if($staffDict['staff_role'] == self::ROLE_EMP) {
				$subject = "【ビジネスdXストア　dXリモート営業】ユーザ招待のお知らせ";
			}

			$message  = "NTTドコモ　ビジネスdXストアをご利用いただきまして、誠にありがとうございます。 \r\n";
			if ($staffDict['staff_role'] == self::ROLE_EMP) {
				$message .= $client["client_name"]. "　管理者様より、dXオンライン営業に招待されました。 \r\n";
				$message .= "※このお知らせの有効期限は24時間です。 \r\n";
				$message .= "※本メールは自動送信されていますので返信はご遠慮ください。 \r\n";
			} else {
				$message .= "お申込みいただきましたサービスのご利用準備が整いましたのでご連絡申し上げます。 \r\n";
			}
			$message .= "※本メールは、お申込みいただきましたdXオンライン営業を提供する株式会社アイドマ・ホールディングスより送信しております。 \r\n";
			$message .= "\r\n";
			$message .= "サービス名：\r\n";
			...
			$message .= "\r\n";
			$message .= "\r\n";
			$message .= $this->partsContactInformationMeetIn(false); // お問い合わせ先の追加.
		}
	...
	public function sendPasswordReissueMail($staffDict){
		...
		$admin_mail = $this->config->mail->admin_mail;
		// タイトル
		$subject =  "【ビジネスdXストア　dXオンライン営業】パスワード再発行のお知らせ";
		// 本文
		$message  = '';
		$message .= "----------------------------------------------------------------- \r\n";
	...
	*/
	...
	Public function partsContactInformationMeetIn($line = true) {
		return
		($line ? "\r\n----------------------------------------------------------------------\r\n" : null).
		"本メールに身に覚えがない場合、お申込み内容に間違いがある場合、\r\n" .
		"他、ご不明な点がありましたら下記までご連絡ください。\r\n" .
		"■ビジネスdXストア　サポートセンター（運営会社 株式会社NTTドコモ）\r\n" .
		"・フォームからのお問い合わせ：\r\n" .
		"　ビジネスdXストア（https://biz-dxstore.docomo.ne.jp/）の「資料請求・お問合せ」からお問合せください。\r\n" .
		"・お電話でのお問い合わせ : 0120-807-062 \r\n" .
		"　※（電話受付時間：土日祝日を除く 平日10：00～18：00）\r\n" .
		"\r\n" .
		"本サービスの操作方法等のお問合せは下記までご連絡下さい。\r\n" .
		"■dXオンライン営業サポートデスク（運営会社 株式会社アイドマ・ホールディングス）\r\n" .
		"　電話 : 0120-467-481 \r\n" .
		"　メール：support@docomodx.aidma-hd.jp \r\n" .
		"　※（電話受付時間：土日祝日を除く 平日10：00～18：00） \r\n".
		($line ? "\r\n----------------------------------------------------------------------\r\n" : null);
	}
...
/**
	...
	* @param unknown $staffDict
	*/
	public function sendStaffInvitationMail($sender, $email, $url) {

		// 送信者メールアドレス
		$admin_mail = $this->config->mail->admin_mail;

		// タイトル
		$subject =  "【ビジネスdXストア　dXオンライン営業】ユーザー招待のお知らせ";
		// 本文
		$message  = '';
		$message .= "----------------------------------------------------------------- \r\n";
		$message .= "招待メールを送信いたします。 \r\n";
		$message .= "-----------------------------------------------------------------\r\n";
		$message .= $sender['client_name']." 管理者様より、dXオンライン営業に招待されました。 \r\n";
		$message .= "サービスを利用するには次のURLをクリックしてビジネスdアカウントでログインしてください。\r\n";
		$message .= "\r\n";
		$message .= $url."\r\n";
		$message .= "\r\n";
		$message .= "\r\n";
		...
		$this->sendMail($admin_mail, $email, $admin_mail, $subject, $message);
	}
...
// 	//-------------------------------------------------------------------------
	...
	private function sendMail($fromname, $sendto, $admin_mail, $subject, $message, $toLeader=null){
		...
		$header .= "From: ".$from_name."<support@docomodx.aidma-hd.jp>\n";
...
URL：https://meet-in.jp/
	...
	public function sendActivationMail($staffDict, $econtractFlg=false) {
		...
		$subject =  "【ビジネスdXストア　dXオンライン営業】アカウントパスワードリマインダのお知らせ";
