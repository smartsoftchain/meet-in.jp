		...
		if ($request->isPost() && Application_CommonUtil::refererCheck(self::REFERER_STAFF_REGIST)) {
			...
			if(count($errorList) == 0){
				...
				try{
					...
					if($sType == 'AA') {
						$mailModel->sendRegistAaAccountMail($action_jp, $staffDict);
					} else if($sType == 'CE') {
						...
						$mailModel->sendRegistCeAccountMail($action_jp, $staffDict, $client);
					}
				...
				// MEMO. docomo案件 連携データの保存.
				if (0 == $collaborativeServicesProvidedDataDao->fetchAllCount('staff_type = "'. $staffDict["staff_type"] .  '" and staff_id = ' . $staffDict["staff_id"]) && $staffDict["accountid"] != null) {
				...
				} else if(!empty($staffDict["staff_type"]) && !empty($staffDict["staff_id"])){

					$collaborativeSql = 'staff_type = "'. $staffDict["staff_type"] .  '" and staff_id = ' . $staffDict["staff_id"];
					if($staffDict["accountid"] == null) {
						// 連携解除.
						$collaborativeServicesProvidedDataDao->delete($collaborativeSql);
					} else {
						// 更新.
						$collaborativeServicesProvidedDataDao->update($collaborativeSql,
						[
							"accountid"              => $form["accountid"],
							"DocomoID"               => $form["DocomoID"],
							"dprofile_email" 		 => $form["dprofile_email"]
...
//				error_log('cond '. $condition . '   join_cond '.$join_condition);
				...
				$collaborativeData = $collaborativeServicesProvidedDataDao->fetchAll('staff_type = "'. $staffDict["staff_type"] .  '" and staff_id = ' . $staffDict["staff_id"]);
				$staffDict["accountid"] = $collaborativeData[0]["accountid"];
				$staffDict["DocomoID"] = $collaborativeData[0]["DocomoID"];
				$staffDict["dprofile_email"] = $collaborativeData[0]["dprofile_email"];
			}else{
				// 新規作成の場合
	...
	public function staffDelete($form, $request){
		...
		if(isset($staffType) && isset($staffId)){
			...
			try{
				// 担当者の論理削除
				$masterStaffDao->deletStaff($staffType, $staffId); // 論理削除.
				$collaborativeServicesProvidedDataDao->deletStaff($staffType, $staffId); // 物理削除(ドコモのデータを極力残したくない).
...
//		}
	...
	function masterStaffValidation($data, $masterStaffDao){
		$validationDict = array(
			"staff_firstname"   =>array("name" =>"名前(姓)", 			"length" => 20, 	"validate" => array(1)),
...
//			"meetin_number"     =>array("name" =>"meetin番号", 		"length" => 11, 	"validate" => array()),
			...
			"DocomoID"          =>array("name" =>"ビジネスdアカウントID", 		"length" => 257, 	"validate" => array(1)),
			"accountid"         =>array("name" =>"アカウント識別子", 	"length" => 24, 	"validate" => array(1,4,18)),
			"dprofile_email" =>array("name" =>"連絡先メールアドレス", 		"length" => 256, 	"validate" => array(1,7)),
		...
		if($data["staff_type"] == "AA") {
			$validationDict['DocomoID']  = array("name" =>"ビジネスdアカウントID", 		"length" => 257, 	"validate" => array());
			$validationDict['accountid'] = array("name" =>"アカウント識別子", 	"length" => 257, 	"validate" => array());
			$validationDict['dprofile_email'] = array("name" =>"連絡先メールアドレス", 		"length" => 256, 	"validate" => array(7));
		}
		...
		if($data["staff_password_mod"] == 1) {
			...
			if(isset($data["staff_password"]) && empty($data["accountid"])) {
		...
		// MEMO. docomo案件 ビジネスdアカウントID を重複登録されると ログインできなくなることから確認する.
		if($data['accountid'] != null) {
			$collaborativeServicesProvidedDataDao = Application_CommonUtil::getInstance("dao", "CollaborativeServicesProvidedDataDao", $this->db);
			$matchs = $collaborativeServicesProvidedDataDao->fetchAll(sprintf('accountid = "%s"', $data['accountid']));
			if(0 != count($matchs)) {
				$match = $matchs[0];
				if(!($match["staff_type"] == $data['staff_type'] && $match['staff_id'] == $data['staff_id'])) {
					$errorList["error"][] = 'このビジネスdアカウントIDは、別のスタッフに結び付けられています';
				}
			}
					...
					if(is_uploaded_file($tmp) && $_FILES['material_file']['error'] == 0) {
						...
						if(move_uploaded_file($tmp, $uploadfile)) {
							if(preg_match('/(gif|jpg|png)/i', $ext, $matches)) {
								...
								$cmd = '/usr/bin/convert -density 200 -units PixelsPerInch -alpha remove '.$pdffile.' '.$uploaddir.md5($result["material"]["material_id"]).'-%03d.jpg';
								// JPG変換開始ログ
		...
		if($request->isPost()) {
			...
			if(count($errorList) == 0) {
				...
				try{
					...
					// メールを送信する
					$mailModel = Application_CommonUtil::getInstance('model', "MailModel", $this->db);
					$mailModel->sendRegistAaAccountMail("編集", $staffDict);
