...
*/
  ...
  **/
  ...
  public function sendApprovalRequest($form) {
    ...
    if(count($partners) != 0) {
      ...
      $mailModel->sendApprovalRequestMail2($partners[0]["email"], $partners[0]["token"], $form["message"], $case['approval_method']);
      $result['status'] = 'sent';
    } else {
  ...
  public function getAggregateCaseIdListPartnerApprovalStatusByClientId($clientId) {
    ...
    if(0 < count($countList)) {
      foreach($countList as $count){
        ...
        } else if($count['null_target_count'] == 0){
          $confirmingIds[] = $count['case_id'];// 「確認中」= target カラムの割当が全て済んでいるもののみ署名依頼メールを送ることが出来るのでそれで判定している.
        }
  ...
  */
   ...
   * @param unknown $form
   */
  public function approvalAndNext($form) {
    // 戻り値を宣言
    $result = array();
  ...
  **/
  public function certificateRegist($form) {
    ...
    try{
     ...
     $this->db->commit();
     $mailModel = Application_CommonUtil::getInstance('model', "MailModel", $this->db);
     $mailModel->sendCertificateRegistMail($form);
    }catch(Exception $e) {
     $this->db->rollBack();
     throw new Exception(sprintf('database error. (%s)', $e->getMessage()));
    }
  ...
  **/
  public function certificateUpdate($certificate) {
    ...
    try{
     ...
     $this->db->commit();
     $mailModel = Application_CommonUtil::getInstance('model', "MailModel", $this->db);
     $mailModel->sendCertificateRegistMail($certificate);
    }catch(Exception $e) {
     $this->db->rollBack();
     throw new Exception(sprintf('database error. (%s)', $e->getMessage()));
    }
  ...
  **/
  public function apiAccountRegist($company) {
    $client = new Zend_Http_Client(self::API_URI.'v1/accounts', array(
      'maxredirects'  => 0,
      'timeout'    => 30)
    );
    ...
    // POSTデータを生成する
    $postDict['email']             = $company['client_staffleaderemail'];
  ...
  ***/
  public function getApiCredential($account) {
    $client = new Zend_Http_Client(self::API_URI.'v1/credentials', array(
      'maxredirects'  => 0,
      'timeout'    => 30)
    );
    $client->setHeaders(array(
