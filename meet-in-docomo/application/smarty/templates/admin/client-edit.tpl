					...
					<table class="mi_table_input_right mi_table_main">
						<tbody>
							...
							<tr>
								...
								<td class="mi_tabel_content">
									<input type="text" class="text_medium" name="client_staffleaderfirstname" id="client_staffleaderfirstname" value="{$clientDict.client_staffleaderfirstname|escape}" placeholder="氏名" />
					...
					<tbody>
						...
						{if $user.staff_type=="AA"}
							<tr>
								...
								<td class="mi_tabel_content">
								  スタンダード
									<select name="plan" id="select_plan">
										{foreach from=$plan item=row}
											<option id="plan_{$row.id}" value="{$row.id}" {if $clientDict.plan_this_month == $row.id}selected{/if}>{$row.maxStaffCount}</option>
										{/foreach}
									</select>
								</td>
							</tr>
						{else}
							<input type="hidden" name="plan" value="{$clientDict.plan_this_month}">
						{/if}
						{if $user.staff_type=="AA"}
							<tr>
						...
						<div class="mi_flt-r">
							<div class="mi_content_title_option">
								...
								{if $addStaffFlg}
									...
									{elseif $user.staff_role == "CE_1"}
									<a href="/admin/send-staff-invitation?{$staffDict.addURL}" class="mi_title_action_btn click-fade">
										<span class="icon-parsonal-puls"></span>
										<span>追加</span>
									</a>
									{/if}
