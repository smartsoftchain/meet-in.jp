...
.mi_breadcrumb_list_material-detail {
	float: right;
	margin-top: 10px;
	margin-right: 70px;
	font-size: 12px;
...
<div id="mi_main_contents">
	...
	<div id="mi_document_display_area">
		<div class="mi_breadcrumb_list_material-detail">
...
-->
		...
		<div class="mi_select_document">
			...
			{elseif ($material.material_type == 3)}
				<video src="/cmn-data/{$material.material_id|md5}.mp4" class="mi_document" style="width:100%" controls/>{$row.material_title}
			{else}
			...
			{foreach from=$materialDetailList item=row}
				<li id="page_{$row.material_id}_{$row.material_page}" data-filename="{$row.material_filename}" data-confirm_del="false">
					...
					<div id="document_{$row.material_id}_{$row.material_page}" class="{if $row.material_tag_flg == 1}mi_select{/if}">
						...
						{/if}
						{if ($material.material_type==0 || $material.material_type == 2)}
							<span class="icon-close delete_checkbox" id="delete_file_{$row.material_id}_{$row.material_page}"></span>
							<!-- <button type="button" class="delete_btn" id="delete_file_{$row.material_id}_{$row.material_page}">削除</button> -->
						{elseif ($material.material_type == 3)}
							<video src="/cmn-data/{$row.material_filename}" class="mi_document" style="width:100%"/>{$row.material_title}
						{/if}
					</div>
				</li>
			{/foreach}
