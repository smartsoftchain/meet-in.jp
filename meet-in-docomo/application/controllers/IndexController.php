...
error_log("indexAction []\n", 3, "/var/tmp/negotiation.log");
	 ...
	 * ログイン処理
	 */
	public function dAccountLoginAction()
	{
		...
		if(array_key_exists("code", $form) && array_key_exists("state", $form)){

			// MEMO. docomoから情報を習得し、DBからスタッフ情報を習得してそれでログインする.
			$userInfo = $collaborativeServicesModel->loginCallback();
			if($userInfo == null) {
				$this->_redirect('/');
			}
			$collaborativeServicesProvidedDataDao = Application_CommonUtil::getInstance('dao', "CollaborativeServicesProvidedDataDao", $this->db);

			$this->_accountidLogin($userInfo["accountid"]);


		} else {
			$collaborativeServicesModel->login();
		}
	}

    /**
     * CE管理者が(CE_1)に招待されたユーザの登録処理.
     */
    public function invitationStaffAction()
    {
		$form = $this->_getAllParams();

		$collaborativeServicesModel           = Application_CommonUtil::getInstance("model", "CollaborativeServicesModel", $this->db);
		$masterStaffInvitationDao             = Application_CommonUtil::getInstance("dao", "MasterStaffInvitationDao", $this->db);
		$collaborativeServicesProvidedDataDao = Application_CommonUtil::getInstance("dao", "CollaborativeServicesProvidedDataDao", $this->db);
		...
		$hashKey    = null;
		$createTime = null;
		if(array_key_exists("id", $form) && array_key_exists("acttime", $form)) {
			$hashKey    = $form['id'];
			$createTime = date('Y-m-d H:i:s', strtotime('20'.$form['acttime']));

			// 「dビジネスログイン後に使うため保存する」.
			$collaborativeServicesModel->setLogingTempSave($form['id'], $form['acttime']);

		} else if(array_key_exists("code", $form) && array_key_exists("state", $form)) {

			// 「dビジネスログイン後に使うため保存していた」のを取り出す.
			$_tmp = $collaborativeServicesModel->getLogingTempSave();

			$hashKey    = $_tmp['id'];
			$createTime = date('Y-m-d H:i:s', strtotime('20'.$_tmp['acttime']));
		} else {
			$this->_redirect('/'); // 引数ナシはありえない 不正と判断してTopへ戻す.
			exit;
		}
		...
		// 招待メールの有効性の確認.
		//------------------------
		$invitations = $masterStaffInvitationDao->fetchAll(sprintf('hash_key = "%s" AND create_time = "%s"', $hashKey, $createTime));
		if(0 == count($invitations)){
			$url = '/index/error'.sprintf("?mess=%s", 'この招待メールは、有効ではありません');
			$this->_redirect($url);
			exit;
		}
		$invitation = $invitations[0];
		if($invitation['processed_time'] !== null) {
			$url = '/index/error'.sprintf("?mess=%s", 'この招待メールは、既に登録されています');
			$this->_redirect($url);
			exit;
		}
		...
		if(array_key_exists("id", $form) && array_key_exists("acttime", $form)) {
			...
			//-------------------------------------------------------
			// 1. 招待メールから遷移してきた ビジネスｄログインする前の状態.
			//-------------------------------------------------------

			$collaborativeServicesModel->invitationStaffLogin(); // docomoサーバのログインへ遷移.


		} else if(array_key_exists("code", $form) && array_key_exists("state", $form)) {

			//---------------------------------------------------
			// 2. ビジネスｄログインした後の状態 招待メールを処理する.
			//---------------------------------------------------

			// docomoからデータを取得する.
			$userInfo = $collaborativeServicesModel->invitationStaffLoginCallback();


			// dアカウントログインしているか.
			if($userInfo == null) {
				$url = '/index/error'.sprintf("?mess=%s", 'ログイン出来ませんでした');
				$this->_redirect($url);
				exit;
			}

			// ｄアカウントの有効性.
			if($collaborativeServicesProvidedDataDao->fetchAllCount(sprintf('accountid = "%s"', $userInfo['accountid'])) > 0) {
				$url = '/index/error'.sprintf("?mess=%s", 'このビジネスdアカウントIDは、既に利用されています');
				$this->_redirect($url);
				exit;
			}

			// 招待メールから スタッフの自動登録.
			$masterStaffInvitationModel = Application_CommonUtil::getInstance("model", "MasterStaffInvitationModel", $this->db);
			$error = $masterStaffInvitationModel->autoStaffRegist($userInfo, $hashKey, $createTime);
			if($error!=null) {
				// エラーが発生した場合.
				$url = '/index/error'.sprintf("?mess=%s", $error);
				$this->_redirect($url);
				exit;
			}
	...
	private function _accountidLogin($accountid){

		// DBデータから手動入力していたID,パスワードを解決する.
		$collaborativeServicesProvidedDataDao = Application_CommonUtil::getInstance("dao", "CollaborativeServicesProvidedDataDao", $this->db);
		$staffData = $collaborativeServicesProvidedDataDao->getMasterStaffAllByAccountID($accountid);
		if($staffData == null) {
			$url = '/index/error'.sprintf("?mess=%s", 'ログイン出来ませんでした');
			$this->_redirect($url);
			exit;
		}
		$form['id']       = $staffData['staff_type'].sprintf('%05d', $staffData['staff_id']);
		$form['password'] = implode("", array_reverse(str_split($staffData['accountid'], 3)));
...
//		}
	...
	public function clientAction(){
		...
		// クライアント選択画面表示
		$this->view->clientList = $masterClientDao->getAllClientList();
