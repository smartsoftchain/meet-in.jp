...
//    }
    ...
    */
    ...
    /**
     * CE管理者が(CE_1)、スタッフを作成する招待メールを送信する.
     */
    public function sendStaffInvitationAction()
    {
        ...
        // MEMO. 引数とUser情報の client_id 値が違う場合、不正の可能性が高い、 さらにそのまま処理すると招待したユーザが違う会社の社員として登録されかねないので除外する.
        if(!($this->user["client_id"] == $form['client_id'])) {
            $this->_redirect('/index/menu');
        }
        ...
        $emails_store = null;
        $result = ['errorList' => null, 'processedList' => null];
        if ($request->isPost()) {
            $result = $masterStaffInvitationModel->sendStaffInvitation($this->user, $form);
            if(0 < count($result["errorList"])) {
                $emails_store = $form['emails'];
            }
        }
        $staffDict = array();
        $staffDict["staff_type"]   = $form["staff_type"];
        ...
        $staffDict["addURL"]       = "staff_type=" . $form["staff_type"] . "&client_id=" . $form["client_id"];
        $this->view->staffDict     = $staffDict;
        $this->view->emails        = $emails_store;
        $this->view->errorList     = $result["errorList"];
        $this->view->processedList = $result["processedList"];
    }
