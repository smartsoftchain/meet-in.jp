...
// START---------------------------------------------
		...
		// ルームロック解除処理
		function (all_peer_id, dbConnection, callback) {
			logger.info("function RoomLock Release");

			dbConnection.beginTransaction(function(err) {
				if (err) {
					logger.error('RoomLock Release beginTransaction error=' + err);
				 ...
				 */
				var sql_init2 = `UPDATE connection_info con
								 LEFT JOIN master_client_new cli
								   ON con.client_id = cli.client_id
								   SET con.room_state =0
								  WHERE con.room_state!=0
								  AND con.updated_datetime < (NOW() - INTERVAL 30 SECOND)
								  AND (con.user_peer_id_1 IS NULL OR con.user_peer_id_1 ='X')
								  AND (con.user_peer_id_2 IS NULL OR con.user_peer_id_2 ='X')
								  AND (con.user_peer_id_3 IS NULL OR con.user_peer_id_3 ='X')
								  AND (con.user_peer_id_4 IS NULL OR con.user_peer_id_4 ='X')
								  AND (con.user_peer_id_5 IS NULL OR con.user_peer_id_5 ='X')
								  AND (con.user_peer_id_6 IS NULL OR con.user_peer_id_6 ='X')
								  AND (cli.negotiation_room_type is null or cli.negotiation_room_type <> '1')
								`;
				logger.debug("RoomLock Release SQL=["+sql_init2+"]");

				dbConnection.query(sql_init2, function (err, result) {
					if (err) {
						logger.error('RoomLock Release error=' + err);
						dbConnection.rollback(function(err) {
							if (err) {
								logger.warn('RoomLock Release rollback error=' + err);
							}
						});	// rollback_end
						callback(new Error(err), all_peer_id, dbConnection);
					}
					...
					dbConnection.commit(function(err) {
						if (err) {
							logger.error('RoomLock Release commit error=' + err);
							dbConnection.rollback(function(err) {
								if (err) {
									logger.warn('RoomLock Release rollback error=' + err);
								}
						...
						logger.info('RoomLock Release result (' + result.changedRows + ')');
						// logger.info("●完了");
		...
		// user_peer_id_1
		function (all_peer_id, dbConnection, callback) {
			logger.info("function user_peer_id_1");
			...
			dbConnection.query(sql1, function (err, result) {
					...
					if( !mConnectionTable[rec.user_peer_id_1] ) {
						...
						dbConnection.beginTransaction(function(err) {
							...
							dbConnection.query(sql11, [id,peerId], function (err, result) {
								if (err) {
									logger.error('SQL11 error=' + err);
									dbConnection.rollback(function(err) {
										if (err) {
											logger.warn('SQL11 rollback error=' + err);
										}
									});	// rollback_end
									callback(new Error(exception), all_peer_id, dbConnection);
								}
								...
								dbConnection.commit(function(err) {
									if (err) {
										logger.error('SQL11 commit error=' + err);
										dbConnection.rollback(function(err) {
											if (err) {
												logger.warn('SQL11 rollback error=' + err);
											}
									...
									logger.info('SQL11 result (' + result.changedRows + ')');
									next();
								}); // commit_end
							});	// query_end
						});	// beginTransaction
				...
				function complete(err){
					if(err) {
						logger.error('SQL11 complete error=' + err);
						callback(new Error(exception), all_peer_id, dbConnection);
					}
					// logger.debug("●●●1完了");
					callback(null, all_peer_id, dbConnection);
		...
		// user_peer_id_2
		function (all_peer_id, dbConnection, callback) {
			logger.info("function user_peer_id_2");
			...
			dbConnection.query(sql2, function (err, result) {
				...
				async.each(result, function(rec, next){
					...
					if( !mConnectionTable[rec.user_peer_id_2] ) {
						logger.info("id=("+ rec.id +") room_name=("+ rec.room_name +") peer_id=("+ rec.user_peer_id_2 +")");
						// logger.info("runAllPeerId::all_peer_id("+ Object.keys(mConnectionTable) +")");
						...
						dbConnection.beginTransaction(function(err) {
							...
							dbConnection.query(sql21, [id,peerId], function (err, result) {
								if (err) {
									logger.error('SQL21 error=' + err);
									dbConnection.rollback(function(err) {
										if (err) {
											logger.warn('SQL21 rollback error=' + err);
										}
									});	// rollback_end
									callback(new Error(exception), all_peer_id, dbConnection);
								}
								...
								dbConnection.commit(function(err) {
									if (err) {
										logger.error('SQL21 commit error=' + err);
										dbConnection.rollback(function(err) {
											if (err) {
												logger.warn('SQL21 rollback error=' + err);
											}
									...
									logger.info('SQL21 result (' + result.changedRows + ')');
									next();
								}); // commit_end
							});	// query_end
						});	// beginTransaction
				...
				function complete(err){
					if(err) {
						logger.error('SQL21 complete error=' + err);
						callback(new Error(exception), all_peer_id, dbConnection);
					}
					// logger.debug("●●●2完了");
					callback(null, all_peer_id, dbConnection);
		...
		// user_peer_id_3
		function (all_peer_id, dbConnection, callback) {
			logger.info("function user_peer_id_3");
			...
			dbConnection.query(sql3, function (err, result) {
				...
				async.each(result, function(rec, next){
					...
					if( !mConnectionTable[rec.user_peer_id_3] ) {
						logger.info("id=("+ rec.id +") room_name=("+ rec.room_name +") peer_id=("+ rec.user_peer_id_3 +")");
						// logger.info("runAllPeerId::all_peer_id("+ Object.keys(mConnectionTable) +")");
						...
						dbConnection.beginTransaction(function(err) {
							...
							dbConnection.query(sql31, [id,peerId], function (err, result) {
								if (err) {
									logger.error('SQL31 error=' + err);
									dbConnection.rollback(function(err) {
										if (err) {
											logger.warn('SQL31 rollback error=' + err);
										}
									});	// rollback_end
									callback(new Error(exception), all_peer_id, dbConnection);
								}
								...
								dbConnection.commit(function(err) {
									if (err) {
										logger.error('SQL31 commit error=' + err);
										dbConnection.rollback(function(err) {
											if (err) {
												logger.warn('SQL31 rollback error=' + err);
											}
									...
									logger.info('SQL31 result (' + result.changedRows + ')');
									next();
								}); // commit_end
							});	// query_end
						});	// beginTransaction
				...
				function complete(err){
					if(err) {
						logger.error('SQL31 complete error=' + err);
						callback(new Error(exception), all_peer_id, dbConnection);
					}
					// logger.debug("●●●3完了");
					callback(null, all_peer_id, dbConnection);
		...
		// SQL4
		function (all_peer_id, dbConnection, callback) {
			logger.info("function user_peer_id_4");
			...
			dbConnection.query(sql4, function (err, result) {
				...
				async.each(result, function(rec, next){
					...
					if( !mConnectionTable[rec.user_peer_id_4] ) {
						logger.info("id=("+ rec.id +") room_name=("+ rec.room_name +") peer_id=("+ rec.user_peer_id_4 +")");
						// logger.info("runAllPeerId::all_peer_id("+ Object.keys(mConnectionTable) +")");
						...
						dbConnection.beginTransaction(function(err) {
							...
							dbConnection.query(sql41, [id,peerId], function (err, result) {
								if (err) {
									logger.error('SQL41 error=' + err);
									dbConnection.rollback(function(err) {
										if (err) {
											logger.warn('SQL41 rollback error=' + err);
										}
									});	// rollback_end
									callback(new Error(exception), all_peer_id, dbConnection);
								}
								...
								dbConnection.commit(function(err) {
									if (err) {
										logger.error('SQL41 commit error=' + err);
										dbConnection.rollback(function(err) {
											if (err) {
												logger.warn('SQL41 rollback error=' + err);
											}
									...
									logger.info('SQL41 result (' + result.changedRows + ')');
									next();
								}); // commit_end
							});	// query_end
						});	// beginTransaction
				...
				function complete(err){
					if(err) {
						logger.error('SQL41 complete error=' + err);
						callback(new Error(exception), all_peer_id, dbConnection);
					}
					// logger.debug("●●●4完了");
					callback(null, all_peer_id, dbConnection);
		...
		// SQL5
		function (all_peer_id, dbConnection, callback) {
			logger.info("function user_peer_id_5");
			...
			dbConnection.query(sql5, function (err, result) {
				...
				async.each(result, function(rec, next){
					...
					if( !mConnectionTable[rec.user_peer_id_5] ) {
						logger.info("id=("+ rec.id +") room_name=("+ rec.room_name +") peer_id=("+ rec.user_peer_id_5 +")");
						// logger.info("runAllPeerId::all_peer_id("+ Object.keys(mConnectionTable) +")");
						...
						dbConnection.beginTransaction(function(err) {
							...
							dbConnection.query(sql51, [id,peerId], function (err, result) {
								if (err) {
									logger.error('SQL51 error=' + err);
									dbConnection.rollback(function(err) {
										if (err) {
											logger.warn('SQL51 rollback error=' + err);
										}
									});	// rollback_end
									callback(new Error(exception), all_peer_id, dbConnection);
								}
								...
								dbConnection.commit(function(err) {
									if (err) {
										logger.error('SQL51 commit error=' + err);
										dbConnection.rollback(function(err) {
											if (err) {
												logger.warn('SQL51 rollback error=' + err);
											}
									...
									logger.info('SQL51 result (' + result.changedRows + ')');
									next();
								}); // commit_end
							});	// query_end
						});	// beginTransaction
				...
				function complete(err){
					if(err) {
						logger.error('SQL51 complete error=' + err);
						callback(new Error(exception), all_peer_id, dbConnection);
					}
					// logger.debug("●●●5完了");
					callback(null, all_peer_id, dbConnection);
		...
		// SQL6
		function (all_peer_id, dbConnection, callback) {
			logger.info("function user_peer_id_6");
			...
			dbConnection.query(sql6, function (err, result) {
				...
				async.each(result, function(rec, next){
					...
					if( !mConnectionTable[rec.user_peer_id_6] ) {
						logger.info("id=("+ rec.id +") room_name=("+ rec.room_name +") peer_id=("+ rec.user_peer_id_6 +")");
						// logger.info("runAllPeerId::all_peer_id("+ Object.keys(mConnectionTable) +")");
						...
						dbConnection.beginTransaction(function(err) {
							...
							dbConnection.query(sql61, [id,peerId], function (err, result) {
								if (err) {
									logger.error('SQL61 error=' + err);
									dbConnection.rollback(function(err) {
										if (err) {
											logger.warn('SQL61 rollback error=' + err);
										}
									});	// rollback_end
									callback(new Error(exception), all_peer_id, dbConnection);
								}
								...
								dbConnection.commit(function(err) {
									if (err) {
										logger.error('SQL61 commit error=' + err);
										dbConnection.rollback(function(err) {
											if (err) {
												logger.warn('SQL61 rollback error=' + err);
											}
									...
									logger.info('SQL61 result (' + result.changedRows + ')');
									next();
								}); // commit_end
							});	// query_end
						});	// beginTransaction
				...
				function complete(err){
					if(err) {
						logger.error('SQL61 complete error=' + err);
						callback(new Error(exception), all_peer_id, dbConnection);
					}
					// logger.debug("●●●6完了");
					callback(null, all_peer_id, dbConnection);
		...
		function (err, results) {
			if (err) {
				logger.error('runAllPeerId error. (' + err + ')');
			}
			else {
