...
$('#case-connection #input-user-name').on('input', function(e) {
  ...
  $('#case-lock-enter-request #input-user-name').val(value);
});
$('#case-lock-enter-request #input-user-name').on('input', function(e) {
  ...
  $('#case-connection #input-user-name').val(value);
});
		...
		.then(function(devices) {
			devices.forEach(function(device) {
				if(0 < device.deviceId.length) {
					...
					if(device.deviceId == use_camera_id) {
			...
			if(!isHaveVideoinput) {
				...
				err_list.push("※カメラがOFFになっているかお使いのブラウザでカメラが許可されていません");
				NEGOTIATION.isMyCameraOn = false;
			}
			if(!isHaveAudioinput){
				a_element = '<a href="#" onclick="openMicQ()" class="tooltip">'+img_element+'</a>';
				err_list.push("※マイクがOFFになっているかお使いのブラウザでマイクが許可されていません");
				NEGOTIATION.isMyMicOn = false;
			}
...
function recommendEnvironmentAlert() {
	...
	RECOMMENDED_CHECK.alertMeasureNetworkSpeedMbps(function(device, mbps) {
		const normal_level = RECOMMENDED_CHECK.NETWORK_SPEED_MBPS_THRESHOLD;
		const low_level    = RECOMMENDED_CHECK.NETWORK_SPEED_MBPS_THRESHOLD_LOW;
