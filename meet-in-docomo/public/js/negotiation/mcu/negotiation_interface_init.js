...
//	mNegotiationMain.destroy(false);
	...
	if (getRoomMemberCount() == 1) {
		$.ajax({
			...
			// 上記でやっていることMeetinCoreManager.js
			// タイマー管理系をクリア、DB更新connetcion_infoのpeeridをnull
			// 画面共有のストリーム破棄、管理テーブルクリア
			leaveRooms();
		});
	} else {
		leaveRooms();
...
//							$('#enter_negotiation_datetime').val(data.datetime);
					...
					var completeCallback = function(XMLHttpRequest, textStatus) {
						// 画面更新せずに再接続
						sendReconnectStart();
						mNegotiationMain.reconnect(reconnectedCallback, reconnectedCallback);
						viewThumbnail();
