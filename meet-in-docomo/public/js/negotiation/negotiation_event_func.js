...
NEGOTIATION.eventBind = function () {
	...
	if (!USER_PARAM_IS_IPAD_PC) { // ipadPCでたまにホバー検知するため一応入れておく
		$('#button_reaction').tooltipster({
			...
			// チップが表示される前に実行される
			functionBefore: function (instance, helper) {
				instance.content('<span class="tooltip_button_reaction" style="letter-spacing: 2.0px;">いいねする</span>');
			},
			functionPosition: function (instance, helper, position) {
	...
	if (!USER_PARAM_IS_IPAD_PC) { // ipadPCでたまにホバー検知するため一応入れておく
		$('#button_raise_your_hand').tooltipster({
			...
			// チップが表示される前に実行される
			functionBefore: function (instance, helper) {
				instance.content('<span class="tooltip_button_raise_your_hand" style="letter-spacing: 2.0px;">挙手する</span>');
			},
			functionPosition: function (instance, helper, position) {
				position.coord.left += 35;
				position.coord.top += 0;
...
$(function () {
	...
	if (getBrowserType() != 'Chrome') {
		...
		var ScreenCaptureSuccessCallback = function (stream, _paramsCb) {
			...
			var _delayTimer = 60000;	// 60秒 60000　秒数は要望に応じて変更可能 10秒 10000がgoogle通知も消えてキャプチャできるほどよい秒数
