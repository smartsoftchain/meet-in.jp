 ...
 */
function onRecognizeStartFunc(initializeEventHandler = (conversationId, result) => {}){
	...
	function onSuccessFunc(stream, speakerNo = 0){
		...
		// APIサーバーへの接続準備
		const conn = new RealTimeAudioRecognize(realtimeRecognizeApiServerUrl);

		// stream情報とspeakerNoを紐づける
		streamAndSpeakerNoRelation(stream.id, speakerNo);
		...
		conn.onmessage = (result) => {
			...
			if(type == "handshake"){
				...
				console.log(result);
				// conversationIdを作成する（Amivoiceの新仕様で追加となったprefixを付与する）
				conversationId = result["conversationIdFirstChar"] + NEGOTIATION_RECORDER.getFileID();
				console.log("conversationId:"+conversationId);

				initializeEventHandler(conversationId, result);
				...
				// チャネルごとに異なる数値を設定
				req["speakerLabel"] = speakerNo;
				// 電話システムがSIPの場合はCallIDを設定
				req["voiceid"] = conversationId+speakerNo;
				// サーバーで生成した通話開始時間を変数へ設定する
				startConversationDateTimes[speakerNo] = result["outgoingDateTime"];
				// 送信するデータをjson化する
				...
				// 個別の音声分析のデータをDBに保持場合はここに記述する
			} else if(type == "message_A"){
				const text = result['text'];

				if (speakerNo === 0) { // speaker0は文字起こしを始めたユーザー
					const myUserName = $("#my_user_info").val();
					streamAndUserNameRelations[stream.id] = {userName: myUserName};
				}
				const userName = streamAndUserNameRelations[stream.id]["userName"];
				const displayText = ` [${userName}]\n ${text}`; // speaker${speakerNo}   ${userName}
				const startTime = result['starttime'] + offsetTime;
				const endTime = result['endTime'] + offsetTime;
				if(text !== '') {
					if(audioTextFlg){
						// 文字起こしを実行中のみ、文字起こしモーダルに文字を書き込む
						getAudioTextData(displayText, startTime);
					}else{
						// 音声分析の場合は、文字データをバッファに貯める
		...
		conn.onclose = () => {
			...
			if(isRunning){
				...
				//document.getElementById('dummy_stop_sentiment_analysis').removeEventListener('click', onRecognizeStopFunc);
		...
		//document.getElementById('dummy_stop_sentiment_analysis').addEventListener('click', onRecognizeStopFunc);
