...
const VIDEO_MORE_SMALL_ICON_PARENT_SIZE_WIDTH = 160;// ビデオ用のより小さいアイコンの大小を決めるwidthしきい値
const VIDEO_MORE_SMALL_ICON_PARENT_SIZE_HEIGHT = 160;// ビデオ用のより小さいアイコンの大小を決めるheightしきい値
const VIDEO_EXTRA_SMALL_ICON_PARENT_SIZE_WIDTH = 110;// ビデオ用の最小アイコンの大小を決めるwidthしきい値
const VIDEO_EXTRA_SMALL_ICON_PARENT_SIZE_HEIGHT = 110;// ビデオ用の最小アイコンの大小を決めるheightしきい値

const DOCUMENT_OFFSET = 5;

const userNamesOriginalWidthForVideo = {}; // リサイズイベントで使用する値を格納していくため、グローバルで持つ
...
LayoutCtrl.prototype = {
	...
	apiSetVideo: function(mode, showVideoArray, hideVideoArray){
		var $targetArea = $("#mi_video_area");
		var tmpW = parent.innerWidth;
		var tmpH = parent.innerHeight;
		var len = showVideoArray.length;

		// ターゲットエリアの設定
		if(tmpW < MINIMUM_WIDTH){
			tmpW = MINIMUM_WIDTH
		}
		if(tmpH < MINIMUM_HEIGHT){
			tmpH = MINIMUM_HEIGHT
		}
		tmpW -= LEFT_TOOL;
		tmpH -= HEAD_AND_FOOT;
		if($targetArea.hasClass("small")){
			tmpW -= this.rightSpace;
		}
		// 資料が飛び出すのでbox_shadow分引く
		tmpW = tmpW - DOCUMENT_OFFSET;
		$targetArea.width(tmpW).height(tmpH);

		var $fitArea = $("#fit_rate");
		var tmpRate = tmpW/tmpH;

		// CSSへの通知クラスをリセットする。クラス名を通じてビデオ状況をCSSに伝える
		$fitArea.removeClass(function(index, className) {
			return (className.match(/\blayout-\S+/g) || []).join(' ');
		});

		// モードに応じてクラス付与
		if(mode == MODE_ONE_BIG){
			$fitArea.addClass("layout-one_big");
		}else{
			$fitArea.addClass("layout-minimum");
		}

		// ビデオ数をクラス付与 マテリアルが表示されていれば,表示数が一個増える
		if($("#negotiation_share_screen").isShow() || $("#mi_docment_area").isShow()){
			if(len <= this.maxVideoNum){
				len += 1;
			}else{
				// 表示限界よりも多いならばマテリアルを閉じる
				saveDocument();
				hideDocument();
				screenOff();
			}
		}
		$fitArea.addClass("layout-num_" + len);

		// 比率に応じて描画する比率を決定する
		if(tmpRate >= CHANGE_SCREEN_DOUBLE_WIDTH){
			tmpRate = CHANGE_SCREEN_DOUBLE_WIDTH;
			$fitArea.addClass("layout-double");
		}else if(tmpRate >= CHANGE_SCREEN_RATE_WIDE){
			tmpRate = CHANGE_SCREEN_RATE_WIDE;
			$fitArea.addClass("layout-wide");
		}else if(tmpRate >= CHANGE_SCREEN_RATE_STANDARD){
			tmpRate = CHANGE_SCREEN_RATE_STANDARD;
			$fitArea.addClass("layout-standard");
		}else{
			tmpRate = CHANGE_SCREEN_RATE_NARROW;
			$fitArea.addClass("layout-narrow");
		}

		// 今の描画領域の比率に合うように比率をセット
		if( tmpW >= tmpH * (tmpRate) ){
			$fitArea.width(tmpH * (tmpRate));
			$fitArea.height(tmpH);
		}else{
			console.debug(tmpRate + " " + tmpW +" " + tmpH);
			$fitArea.width(tmpW);
			$fitArea.height(tmpW / (tmpRate));
		}

		// shiftで値を書き換えるため、参照を消したいので実の値だけコピー
		var showVideoArrayCopy = $.extend(true, [],showVideoArray);
		var hideVideoArrayCopy = $.extend(true, [],hideVideoArray);

		this.resetHeaderVideo(showVideoArrayCopy,hideVideoArrayCopy);

		// ビデオを非表示にして、ヘッダーをアクティブにする
		this.videoHide(hideVideoArrayCopy);




		// 表示する要素がなければおわり
		if(typeof showVideoArrayCopy === "undefined"|| showVideoArrayCopy.length == 0){
			// 手動ですべてヘッダーにいれて何も表示されていないときは、背景に「ビデオをドラッグ & ドロップで表示させてください」とテキストで入れる。
			$('.mi_contents_background_info_content').show();
			return;
		} else {
		...
		// ビデオに順番を付与して表示する
		this.videoShow(mode,showVideoArray);

		// フレーム内のボタン更新
		this.updateVideoIcon(showVideoArray);
	...
	videoHide : function(hideVideoArray){
		...
		if(hideVideoArray.length > 0){
			...
			$.each(hideVideoArray,function(){
				...
				// 非アクティブの値や方法変更の場合は
				// negotiation_event_func.jsのVideoフレーム内(右)、拡大ボタン押下内 + updateRoomMembersWrap内のActiveVideo数のカウントの仕方も併せて変更すること。
...
//			this.css("display", "block");
			...
			if (left !== 'none') {
				this.css('left', left);
				// 画面共有中にモニタリングモードの方が画面更新するとclassが外れて受信側のvideoが表示されない現象が起こる
				// videoのwidth,heightが取得できていない時はリセットする
				let width = this.css('width');
				if (width == "0px") {
					resetShowVideoFlg = true;
				}
				$('#negotiation_target_video_old_left_' + data_id).val("none");
			}
		});

		if (resetShowVideoFlg) {
	...
	/**
	 * ターゲットと親を渡した際、親要素の縦の大きさに応じて、子要素に「.small」や「.more_small」「.extra_small」を付与する関数
	...
	updateVideoIcon : function (showVideoArray){
		function delay() {
			...
			$.each(showVideoArray, function () {
				var data_id = this.attr('data-id');
				const isVideOn = $('#negotiation_target_video_relative_no_video_' + data_id).css('display') === 'none';
				...
				if (this.hasClass('big')) {
					...
					this.find(".mi_video_icon_wrap").removeClass("more_small");
					this.find(".mi_video_icon_wrap").removeClass("extra_small");
					// full-screenアイコン画像を元に戻す
					this.find(".video_full-screen_icon img").attr("src", "/img/all-full-screen.png");
					// マイクoff表示があるとき、ユーザー名表示箇所 大アイコン用のスタイルに書き換え
					if (isVideOn && $(".video_mic_off_icon").css("display") === "block") {
						$('#negotiation_target_video_' + data_id).find('.on_video').css("left", "35px");
					}
				} else {
					...
					// videoオン かつマイクoff表示があるとき、ユーザー名表示箇所 小アイコン用のスタイルに書き換え
					if (isVideOn && $(".video_mic_off_icon").css("display") === "block") {
						$('#negotiation_target_video_' + data_id).find('.on_video').css("left", "28px");
					}

					// extra_smallへの切り替え判定 
					if ($(this).width() < VIDEO_EXTRA_SMALL_ICON_PARENT_SIZE_WIDTH
						|| $(this).height() < VIDEO_EXTRA_SMALL_ICON_PARENT_SIZE_HEIGHT) {
						// 最小アイコンへ
						this.find(".mi_video_icon_wrap").removeClass("small");
						this.find(".mi_video_icon_wrap").removeClass("more_small");
						this.find(".mi_video_icon_wrap").addClass("extra_small");
						// full-screenアイコン画像だけ枠からはみ出るため、小さい画像に差し替える
						this.find(".video_full-screen_icon img").attr("src", "/img/all-full-screen-more-small.png");
						// ビデオオン時のマイクオフアイコンと名前表示が少し隠れてしまうため、微調整
						this.find(".other_video_mic_off_icon, .video_mic_off_icon, .video_user_name_wrap.on_video")
						.addClass("extra_small"); // 他人と自分のマイクオフアイコン、videoオン時のみ名前表示箇所

					// more_smallへの切り替え判定
					} else if ($(this).width() < VIDEO_MORE_SMALL_ICON_PARENT_SIZE_WIDTH 
						|| $(this).height() < VIDEO_MORE_SMALL_ICON_PARENT_SIZE_HEIGHT) {
							// もっと小さいアイコンへ
							this.find(".mi_video_icon_wrap").removeClass("small");
							this.find(".mi_video_icon_wrap").removeClass("extra_small");
							this.find(".other_video_mic_off_icon, .video_mic_off_icon, .video_user_name_wrap.on_video")
								.removeClass("extra_small"); // 他人と自分のマイクオフアイコン、videoオン時のみ名前表示箇所
							this.find(".mi_video_icon_wrap").addClass("more_small");
							// full-screenアイコン画像だけ枠から少しはみ出るため、小さい画像に差し替える
							this.find(".video_full-screen_icon img").attr("src", "/img/all-full-screen-more-small.png");
					} else {
						this.find(".mi_video_icon_wrap").removeClass("extra_small");
						this.find(".mi_video_icon_wrap").removeClass("more_small");
				...
				if(ROOM_MODE_2 == $('#room_mode').val()) {
					 ...
					 // MEMO 現状 は「消音」だけ使える程度.
					this.find(".video_mute_audio").show();

					// MEMO ヘッダーエリアのボタンがモニタリング中 全て操作不能になっていることから「配置を元に戻す」ことが出来ない
					// 触らせなくする為 隠している 仕様が変わったら　隠すと逆に片手落ちになる点に注意し　使えうようにすることになるかもしれないね.
					this.find(".video_big_icon").hide();
					this.find(".video_small_icon").hide();
					this.find(".video_remove_icon").hide();
					this.find(".video_full-screen_icon").hide();
					this.find(".video_full-screen_icon").hide();
					this.find(".video_card_icon").hide(); // あとから入室したユーザが、procSendLogin()で名刺を表示状態にすることがある.
					this.find(".stream_reconnect_icon").hide();
					this.find(".stream_reconnect_icon").hide();
				}
...
/**
 *  video幅とvideo内ユーザー名表示箇所の width調整をする処理
 * @param dataId number dataId(各userId)
 * @param userNamesOriginalWidthForVideo object dataIdと描画初期のwidthを格納したdictionary
 */
function adjustWidthInVideo(dataId, userNamesOriginalWidthForVideo) {

		let videoFrameWidth = $("#negotiation_target_video_" + dataId).width();
		let originalWidth = userNamesOriginalWidthForVideo[dataId];
	
		// originalWidthでmaxWidthを指定
		...
		const retinaSwitch = window.devicePixelRatio; // iPad用 Retina判定用
		let adjustForRetina = 0; // iPad調整用
		...
		if ($('#negotiation_target_video_relative_no_video_' + dataId).css('display') === 'block') {
			...
			// カメラon かつ マイクoffアイコン表示に 判定 兼 video幅との調整値を変更する
			if (isShowIcon) {
					defaultAdjustWidth = 70;
			}
		}

		let belowOriginalWidthFlg = false;
		if (userNameAreaWidth < originalWidth) {
			//　再接続した際に、width0や低い値が入ることから、max-widthより下回らないようにする。
			userNameAreaWidth = originalWidth;
			belowOriginalWidthFlg = true;
		}
			
		if(retinaSwitch == 2) {
			adjustForRetina = 30;
			if (belowOriginalWidthFlg) {
				adjustForRetina = 0;
			}
			// 拡大処理Retina（主にiPad用）
			if ((videoFrameWidth > userNameAreaWidth + defaultAdjustWidth) 
				&& userNameAreaWidth <= (originalWidth - adjustForRetina)
			) {
				$("#negotiation_target_video_" + dataId).find(".no_video").width(userNameAreaWidth + 5);
				$("#negotiation_target_video_" + dataId).find(".on_video").width(userNameAreaWidth + 5);
			}
...
// 文字数から必要幅を計測する処理
function calculateWidthFromString(targetString) {
