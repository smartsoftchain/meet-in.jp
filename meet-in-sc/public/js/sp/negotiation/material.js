...
function getMousePointTag(drowX, drowY, userId){
	// 強調ポインター
	var mouserImg = '<span id="mouse_point_'+userId+'" class="icon-pointer" style="position:absolute;z-index:26;left:'+drowX+'px;top:'+drowY+'px;"></span>';
	return mouserImg;
}
		...
		if(localMaterialId != currentDocumentId){
			if(window.confirm("資料をサムネイルから削除します。\n手書きの情報は失われます。（ゲスト側も削除されます）")){
				...
				$.ajax({
					...
					type: "POST",
					dataType: "json",
					data: {connectionInfoId: connectionInfoId, materialIds : localMaterialId},
				}).done(function(res) {
				});
...
function receiveDocumentJson(json){
	if(json.type == "INIT_DOCUMENT"){
		...
		if($("div#sp_document_area").is(':visible') && currentDocumentId != 0){
			var canvasImg = new Image();
			canvasImg.onload = function(){
				viewDocumentImage(canvasImg, spCanvasUrl, "#sp_img_canvas");
			}
			canvasImg.src = spCanvasUrl;
			/*
			// 資料の表示割合を計算する
			var proportionSize = getProportion();
			// 線を描いた人の比率と自分の比率を考慮し計算を行う
			var proportionDrowX = json.drowX * proportionSize;
			var proportionDrowY = json.drowY * proportionSize;
			var proportionOnMouseX = json.onMouseX * proportionSize;
			var proportionOnMouseY = json.onMouseY * proportionSize;
			...
			if(json.selectLeftTool == TOOL_CURSORE_EMPHASIS){
				// カーソル要素が存在するか調べる
			...
			if(json.writeLineFlg){
				// 線を描いた人の比率と自分の比率を考慮し計算を行う
				writeLine("sp_img_canvas", proportionDrowX, proportionDrowY, proportionOnMouseX, proportionOnMouseY, json.line, json.color, json.selectLeftTool);
			}
			*/
		}
