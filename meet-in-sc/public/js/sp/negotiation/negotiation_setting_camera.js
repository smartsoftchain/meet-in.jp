...
console.log("カメラIDが切り替:("+ NEGOTIATION.use_camera_facingMode +")");
	...
	if ('Chrome' === USER_PARAM_BROWSER) {
		...
		$targetVideo.css('filter','brightness('+brightVal+'%)');
	}
...
console.log("getCameraConstraints0：use_camera_facingMode["+ NEGOTIATION.use_camera_facingMode +"]");
	...
	if (NEGOTIATION.use_camera_id && NEGOTIATION.use_camera_id != 'null') {
		// モバイル対応
		if (USER_PARAM_BROWSER === 'Safari' && USER_PARAM_IS_MOBILE) {
console.log("facingMode["+ NEGOTIATION.use_camera_facingMode +"]");
			config.video.facingMode = {exact: NEGOTIATION.use_camera_facingMode};
		}
		else {
console.log("getCameraConstraints2：use_camera_id["+ NEGOTIATION.use_camera_id +"]");
			config.video.deviceId = {exact: NEGOTIATION.use_camera_id};
		}
console.log("getCameraConstraints3：use_camera_id["+ NEGOTIATION.use_camera_id +"]");
	} else {
	...
	if (NEGOTIATION.use_fps) {
		...
		else if (USER_PARAM_BROWSER === 'Safari') {
