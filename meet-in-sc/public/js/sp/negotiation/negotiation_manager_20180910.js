...
//			mUserIdAndSecretKey[mUserId] = USER_SERECT_KEY;
...
/*
		...
		// 特定な相手に接続する
		NegotiationMain.prototype.connectToTarget = function(
				target_user_id,
				target_peer_id,
				dataConnectionCloseCallback,
				coreCommandCallback,
		...
		var reconnect = function(
			...
			if (mMeetinCore) {
				for (var i = 0; i < MEETIN_MAIN_MAX_PEOPLE; ++i) {
					if (i != mUserId) {
//						meetinFlashTargetVideo_endSession(i);
...
//			USER_SERECT_KEY = createRandomString(10);
		};
...
/*
		...
		var sendBinary = function(
			...
			if (mMeetinWebSocketProc) {
				mMeetinWebSocketProc.sendBinary(
					binary
				);
			}
		...
		// コマンド送信
		NegotiationMain.prototype.sendBinary = function(
				binary
			) {
			return sendBinary(
				binary
...
/*
		NegotiationMain.prototype.closeDataConnection = function(
				target_peer_id
			) {
			if (mMeetinCore) {
				mMeetinCore.closeDataConnection(
					target_peer_id
				);
			}
			if (mMeetinWebSocketProc) {
				mMeetinWebSocketProc.closeDataConnection(
					target_peer_id
				);
			}
...
/*
			} else if (mMeetinCore && mMeetinWebSocketProc) {
				var list1 = mMeetinCore.getConnectedUserIdList();
				var list2 = mMeetinWebSocketProc.getConnectedUserIdList();
				var list = {};
...
/*
		...
		var changePeerDataIdArrayToUserIdArray = function(
			...
			if (mMeetinCore) {
			return mMeetinCore.changePeerDataIdArrayToUserIdArray(
				peer_data_id_array
			);
			}
		...
		// ピアIDの配列をuserIdの配列に変換
		NegotiationMain.prototype.changePeerDataIdArrayToUserIdArray = function(
				peer_data_id_array
			) {
...
/*
		...
		var isConnectedToTargetPeerData = function(
			...
			if (mMeetinCore) {
			return mMeetinCore.isConnectedToTargetPeerData(
				target_peer_id
			);
			}
		...
		// 指定した接続先との接続状態を取得する
		NegotiationMain.prototype.isConnectedToTargetPeerData = function(
				target_peer_id
			) {
		...
		// 指定した接続先との接続状態を取得する
		var isConnectedToUserId = function(
				target_user_id
			) {
			if (mMeetinCore) {
			return mMeetinCore.isConnectedToUserId(
				target_user_id
			);
			}
		...
		// 指定した接続先との接続状態を取得する
		NegotiationMain.prototype.isConnectedToUserId = function(
				target_user_id
			) {
		...
		NegotiationMain.prototype.getPeerDataId = function(
			) {
			if (mMeetinCore) {
			return mMeetinCore.getPeerDataId();
			}
...
/*
		NegotiationMain.prototype.closeAllDataConnection = function(
			) {
			if (mMeetinCore) {
			return mMeetinCore.closeAllDataConnection();
			}

			return 0;
		};

		NegotiationMain.prototype.closeAllMediaConnection = function(
			) {
			if (mMeetinCore) {
			return mMeetinCore.closeAllMediaConnection();
			}

			return 0;
		};

		NegotiationMain.prototype.closeMediaConnectionByStreamId = function(
				streamId
			) {
			if (mMeetinCore) {
			return mMeetinCore.closeMediaConnectionByStreamId(streamId);
			}
