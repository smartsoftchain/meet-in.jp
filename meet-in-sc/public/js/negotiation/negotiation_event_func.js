...
*/
	...
	//			},
			...
			$('.ui-widget-overlay').addClass('negotiation-userlist-dialog-overlay');
...
//			console.log("shareScreenWidth:"+shareScreenWidth+" | shareScreenHeight:"+shareScreenHeight);
	...
	$(document).on('click', '[name=btn_guest_end]', function(){
		...
		.done(function(res) {
			...
			if( refererHost.indexOf('sales-crowd.jp') != -1 ) {
				// SCからの場合はCloseする
				window.open('about:blank','_self').close();
			}
			else {
				// SC以外の場合はメッセージを表示する
				win1 = window.open('about:blank','_self');
				win1.document.write("<html><body>");
				win1.document.write("<p><center>本日はお忙しいところ</center><br>");
				win1.document.write("<center>誠にありがとうございました。</center><br>");
				win1.document.write("<center>タブを閉じて終了してください。</center></p>");
...
//	console.log('click button_room_share');
	...
	$(".mi_modal_shadow").css('background','rgba(120, 120, 120, 0.5)');
	$(".mi_modal_shadow").attr('id','room-share-close');
