...
$(function () {
	...
	// 文字サイズ変更イベント
	// typeが1の場合は2に変更し、2の場合は3に変更し、3の場合は1に変更する
	$(document).on('click', 'span.share_memo_change_fontsize', function(){
		var data = {
				command : "SHARE_MEMO",
	...
	$(document).on('click', 'span.share_memo_download', function(){
		...
		xhr.open("POST", "/negotiation/download-share-memo-pdf", true);
		...
		xhr.onload = function(e) {
			...
			var now = new Date();
			var pdfFileName = "share_memo_"+now.getFullYear()+(now.getMonth() + 1)+now.getDate()+now.getHours()+now.getMinutes()+now.getSeconds()+".pdf";
			// IEの場合はバイナリーのダウンロード方法が変わるので処理を分岐させる
			if(getBrowserType() != "IE"){
				// ファイル名を指定する
				$("a[name=download_share_memo]").attr("download", pdfFileName);
	...
	$('textarea.share_memo_text').bind('touchstart', function() {
		...
		if(!$('textarea.share_memo_text').is(':focus')){
