...
function MediaConnection(peer, provider, options) {
  ...
  this.video_bandwidth = null;
  this.audio_bandwidth = null;
  this.media_type = null;
  this.extra1 = null;
  this.extra2 = null;
  if (this.options) {
    if (this.options.video_bandwidth || 0 === this.options.video_bandwidth) {
      this.video_bandwidth = this.options.video_bandwidth;
    }
    if (this.options.audio_bandwidth || 0 === this.options.audio_bandwidth) {
      this.audio_bandwidth = this.options.audio_bandwidth;
    }
    if (this.options.media_type) {
      this.media_type = this.options.media_type;
    }
    if (this.options.extra1) {
      this.extra1 = this.options.extra1;
    }
    if (this.options.extra2) {
      this.extra2 = this.options.extra2;
    }
  }
...
MediaConnection.prototype.answer = function(stream, options) {
  ...
  this.video_bandwidth = null;
  this.audio_bandwidth = null;
  this.media_type = this.options._payload.media_type;
  this.extra1 = this.options._payload.extra1;
  this.extra2 = this.options._payload.extra2;
  if (options) {
    if (options.video_bandwidth || 0 === options.video_bandwidth) {
      this.video_bandwidth = options.video_bandwidth;
    }
    if (options.audio_bandwidth || 0 === options.audio_bandwidth) {
      this.audio_bandwidth = options.audio_bandwidth;
    }
  }
    ...
    if (!candidate || candidate.candidate === null) {
      ...
      provider.socket.send({
        ...
        payload: {
          ...
          extra1: connection.extra1,
          extra2: connection.extra2
        },
        dst: peerId
      });
    }
...
Negotiator._makeOffer = function(connection) {
  ...
  pc.createOffer(function(offer) {
    ...
    if (connection.video_bandwidth || 0 === connection.video_bandwidth) {
      offer.sdp = util.setMediaBitrate(offer.sdp, 'video', connection.video_bandwidth);
    }
    if (connection.audio_bandwidth || 0 === connection.audio_bandwidth) {
      offer.sdp = util.setMediaBitrate(offer.sdp, 'audio', connection.audio_bandwidth);
    }
...
Negotiator._makeAnswer = function(connection) {
  ...
  pc.createAnswer(function(answer) {
    ...
    if (connection.video_bandwidth || 0 === connection.video_bandwidth) {
      answer.sdp = util.setMediaBitrate(answer.sdp, 'video', connection.video_bandwidth);
    }
    if (connection.audio_bandwidth || 0 === connection.audio_bandwidth) {
      answer.sdp = util.setMediaBitrate(answer.sdp, 'audio', connection.audio_bandwidth);
    }
...
/*change log*/
  ...
  connection.pc.addIceCandidate(new RTCIceCandidate({
    ...
    candidate: candidate
  	...
  	console.log(e);
  	window.postMessage({type: 'ADD_ICE_CANDIDATE_ERROR' },'*');
  });
...
function Peer(id, options) {
  ...
  if (!util.supports.audioVideo && !util.supports.data ) {
    this._delayedAbort('browser-incompatible', 'お使いになるブラウザがWebRTCをサポートしていません。');
    return;
  }
