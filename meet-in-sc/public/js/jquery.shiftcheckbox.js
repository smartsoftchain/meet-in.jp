...
(function ($) {
    ...
    $.fn.shiftcheckbox = function()
    {
        ...
        this.bind("click", function(event) {
            ...
            var checkStatus = $(this).find('[type=checkbox]').is(':checked');
    ...
    $.fn.shiftcheckboxext = function()
    {
        ...
        // tdセルクリック
        $(this).parents('tr').children('td').bind("click", function(event) {
        	if (prevCheckBox) {
        		prevCheckBox = false;
        		return;
        	}
        	var self = $(this).parents('tr').find('td.inp > [type=checkbox]');
        	var val = self.val();
        	var checkStatus = !self.is(':checked');
            if ( checkStatus == undefined ) checkStatus = false;
            ...
            if (event.shiftKey) {
                if (prevChecked[classname] != null) {  // if check 'end point' with ShiftKey
                    ...
                    //get the current checkbox number
                    var ind = 0, found = 0, currentChecked;
                    currentChecked = getSelectedExt(val,classname);
    
                    if (currentChecked < prevChecked[classname]) {
                        $(selectorStr[classname]).each(function(i) {
                            if (i > currentChecked && i < prevChecked[classname]) {
                            	$(this).prop('checked' , startStatus[classname]).trigger("change");
                            }
                        });
                    } else {
                        $(selectorStr[classname]).each(function(i) {
                    ...
                    prevChecked[classname] = getSelectedExt(val,classname);
                    endChecked[classname]  = null;
                    startStatus[classname] = checkStatus;
                }
            } else {                                   // considered to be 'start point'(if not press ShiftKey)
        ...
        $(this).parents('tr').children('td').bind("keyup", function(event) {
            if (endChecked[classname]) {
                    prevChecked[classname] = null;
            }
        });
    ...
    function getSelectedExt(val,classname)
    {
        var ind = 0, found = 0, checkedIndex;

        $(selectorStr[classname]).each(function(i) {
            if (val == $(this).val() && found != 1) {
                checkedIndex = ind;
                found = 1;
            }
            ind++;
