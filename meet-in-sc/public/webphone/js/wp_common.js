...
//get credit from EVENT messages
	...
	if (globalStatus == 'Incoming...')
	{
		...
		if(typeof otherIncomingCall == "function") {
			otherIncomingCall();
		}
	}
	...
	if (((null != voicemailNr) && voicemailNr.length > 0) && checkCall == false)
	{
		if (!wp_api.isFullyCustomized)
		{
			wp_common.ChangeHoldToVoicemail(true);
...
initcheck: function ()	// getting applet handle
{
	...
	try{
		...
		if (isIE()) // for IE the values of strApplethandle is the same regardless if the applethandle is found or not
		{
...
//			width: 290, height: 250
		...
		var errMsg = "<div id=\"javaerrormsg\" style=\"clear:both; padding:5px; display:" + errmsgVisibility + "; background:#fff; color:#f00; font-size:14px; font-weight:bold;\">Javaを有効にするか、<br><a href=\"http://java.com/en/download/index.jsp\" target=\"_blank\">こちら</a>からJavaをインストールをする必要があります。</div>"

		var errMsg2 = '<b>Javaを有効にするか、<br><a href=\"http://java.com/en/download/index.jsp\" target=\"_blank\">こちら</a>からJavaをインストールをする必要があります。</b>';
...
voipDelayedRegister: function ()	//	making delay for API_ServerInit()
{
	...
	//alert('voipDelayedRegister#isRegistered='+isRegistered+' '+ wp_api.parameters['serveraddress']+ ' ' +  wp_api.parameters['username'] + ' ' +  wp_api.parameters['password']);
...
voipStatusRegister: function ()	//	register if double click on status and not in call
{
		...
		&& (wp_api.parameters['password'] !== null) && (wp_api.parameters['password'].length > 0))
	{
		wp_common.onLogin();
		wp_common.pages(2);
		if(!wp_common.initcheck()) {alert('Cannot find applet handle !6');return false;}
...
reRegister: function ()	//	calls API_Register() every "reRegisterTime" miliseconds
{
	if ((boolReRegister != null) && (currentPage == 2))
	{
		if ((checkCall == false) && (wp_api.parameters['username'] !== null) && (wp_api.parameters['username'].length > 0)
			&& (wp_api.parameters['password'] !== null) && (wp_api.parameters['password'].length > 0))
		{
			if(!wp_common.initcheck()) {alert('Cannot find applet handle !7');return false;}
			applethandle.API_Register(wp_api.parameters['serveraddress'], wp_api.parameters['username'], wp_api.parameters['password']);
		}
	}
...
{
	if(!wp_common.initcheck()) {alert('Cannot find applet handle !8');return false;}
	var boolCall = false;
...
wp_RegisterCallHangup: function () // used for click to call
{
    ...
    if(!wp_common.initcheck()) {
    	// 20141016 ikemen
    	//alert('Cannot find applet handle !9');
    	return false;
    }
...
wp_Chat: function (peer)
{
	...
	if(!wp_common.initcheck()) {alert('Cannot find applet handle !17');return false;}

	return applethandle.API_Chat(chatId);
},
wp_AudioDevice: function ()
{
	if(!wp_common.initcheck()) {alert('Cannot find applet handle !18');return false;}
	return applethandle.API_AudioDevice();
},
wp_Mute: function (muteParam)
{
	if(!wp_common.initcheck()) {alert('Cannot find applet handle !19');return false;}
	var muteValue = !mute;
