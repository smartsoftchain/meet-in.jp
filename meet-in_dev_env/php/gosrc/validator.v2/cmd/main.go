...
func writer(dataCh chan IndexedData, wg *sync.WaitGroup, limit int64, sortKeys []string) {
	...
	for {
		...
		errPos = append(errPos, int(indexedData.id))
		counter++
	}
	bs = append(bs, []byte(`],"count":`)...)
	bs = append(bs, []byte(strconv.FormatInt(counter, 10))...)
	bs = append(bs, []byte(`,"errPos":[`)...)
	sort.Ints(errPos)
	errPosBytes, _ := json.Marshal(errPos)
	bs = append(bs, errPosBytes...)
	bs = append(bs, []byte(`]}`)...)

	fileName := string(append(uuid.Must(uuid.NewV4()).Bytes(), []byte(".json")...))
